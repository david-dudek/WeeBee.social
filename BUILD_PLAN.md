...
**1.3 [FOUNDER] Register the domain.**
1. Pick a domain name (this can take longer than any coding step; a bland name is fine for the prototype era — it can change later, users log in by email and never type URLs after bookmarking).
2. Register it at **Porkbun** (porkbun.com — at-cost pricing, free WHOIS privacy, clean interface). Namecheap is an acceptable alternative.
3. **Do not put the domain behind Cloudflare's proxy service** now or later. Cloudflare-the-registrar/DNS is fine, but its orange-cloud "proxy" mode would terminate HTTPS at Cloudflare, meaning a third party decrypts all user traffic — a violation of the privacy architecture (ARCHITECTURE §2, Decision 2). DNS-only, always.
✅ *Done when:* you own the domain and can see its DNS management page.
...
**6.2 [AI] Feed posts.** Composer with audience picker (individuals + groups, live count, hard ≤30 with honest narrowing prompt — SPEC §7.3), one optional image, URL allowlist enforcement at composition (§7.2), audience snapshot rows (§7.4).
...
**13.3 [AI] Django admin as operator console.** Moderation queue with act-on actions (delete content / warn / ban), vocabulary editor, URL-allowlist editor, `NAME_BLOCKLIST` editor (SPEC §4.5), `REACTION_SET` and `THEME_SET` editors; admin at a non-default path (ARCHITECTURE §7).
