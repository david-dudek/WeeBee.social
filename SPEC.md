...
Fixed layout (no freeform customization), with limited theming: font choice and color scheme from operator-curated sets (`THEME_SET`; **every scheme must meet the WCAG 2.1 AA contrast ratios of §16.3 in every combination in which it can appear** — this is why theming is curated rather than freeform). Sections, top to bottom: identity header (photo, display name, **short bio**, hashtags) → pinned posts → static about section (**extended bio**) → image gallery → the blog (the owner's profile posts, newest first, filtered per viewer). The two bio fields, their caps and their differing rules are §9.4; the profile carries no structured biographical fields at all (§9.6).
...
| Viewer | Sees |
|---|---|
| **Friends** | Everything: both tiers, the full blog (all profile posts), gallery, and the extended bio (§9.4). |
| **FoFs (any mutual friend exists)** | **Basic tier only:** display name, profile photo, **short bio** (§9.4), profile hashtags, and the specific mutual friends they share with the owner ("knows Alice and Tom"). |
...
### 9.4 Limits, and the two bio fields (v1.15)
- **Short bio** (identity header): `BIO_SHORT_MAX` = **200 characters**. Visible at the basic tier, to FoFs as well as friends (§9.2), and therefore the one profile field that *travels* — it appears in the friend-request card (§5.2) and wherever basic-tier context is rendered. Three rules follow from that exposure: it is **screened at every save** against `NAME_BLOCKLIST` as display names are (§4.5); it is rate-limited by `BIO_CHANGE_COOLDOWN_HOURS` (§13.6); and it **never renders clickable links**, not even allowlisted ones — URLs typed into it display as inert plain text. There is no use case the contact card does not serve better (§10.2), and a field with no links is a field that cannot deliver one.
- **Extended bio** (the static about section): `BIO_EXTENDED_MAX` = **2,000 characters**, matching `COMMENT_LENGTH_MAX` and the blog fold threshold so it needs no fold rule of its own. **Friends only** (§9.2), so it is not a push surface: it is **not** rate-limited, and allowlisted links **are** permitted in it (§7.2.3). Screened at save as the short bio is.
...
- **Clearing the short bio to empty is never rate-limited**, and a save rejected by screening never consumes the cooldown (§13.6).
...
| Profile photo changed | **Yes** | Recognition-relevant, socially meaningful, low-frequency |
| New gallery image | **Yes, coalesced** | Bounded at `GALLERY_MAX` = 8; a batch upload must never become eight notifications |
...
1. **Structural** (does most of the work): invite-only vouched entry, no reach, bounded audiences, no strangers, **no free-text messaging channel** — friend requests, introductions and reactions carry no composed text (§5.2, §5.5, §8.2) and there is no DM system at all (§10.1) — and the URL allowlist (§7.2.3).
   **One honest qualification (v1.15):** this layer is often summarized as "no free-text vectors," and that overstates it. A friend request displays the requester's **own short bio and profile photo** (§5.2, §9.2), both author-chosen, to someone who did not ask for them. That surface is governed not by the absence of text but by screening at every save, a change cooldown (§13.6), and a no-links rule on the short bio (§9.4). The distinction matters because §4.6.1's anti-phishing argument leans on this layer, and an overstated claim is a weak foundation.
...
**Short-bio and profile-photo change cooldown (v1.15).** The short bio and the profile photo are the only author-controlled content the platform *pushes* toward people who are not the author's friends: both appear in the friend-request card shown to up to 20 FoFs a day (§5.2, §9.2), whether or not the recipient accepts. Without a limit, the pattern is obvious — write a message into the bio, send a batch of requests, rewrite, send another batch — and the field becomes a rotating billboard aimed at people who never opted in. Both are therefore limited to **one change per `BIO_CHANGE_COOLDOWN_HOURS` = 12**, with free re-saves for `BIO_EDIT_GRACE_MINUTES` = 15 measured from the *first* save in a burst, so the clock never restarts and an immediate typo fix costs nothing. Three carve-outs, each deliberate: **clearing the short bio to empty is never rate-limited** (retraction always stays open — the §10.3 principle that the recoverable direction is the free one); a save **rejected by screening does not consume the cooldown**, since a save that never happened cannot start a clock; and the **extended bio is exempt entirely**, being friends-only with no push surface (§9.4). Note what this control is and is not: a rate limit needs no judgment about content, which is why it is the only measure that can cover the **profile photo** — screening cannot read an image, and image moderation is out of scope for a solo operator (§2).
...
| `BIO_CHANGE_COOLDOWN_HOURS` ✎ | 12 (short bio and profile photo; extended bio exempt; clearing to empty never limited) | §9.4, §13.6 |
| `BIO_EDIT_GRACE_MINUTES` ✎ | 15 (free re-saves, measured from the first save in a burst; clock never restarts) | §9.4, §13.6 |
