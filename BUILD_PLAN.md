...
**6.2 [AI] Feed posts.** Composer with audience picker (individuals + groups, live count, hard ≤30 with honest narrowing prompt — SPEC §7.3), one optional image, audience snapshot rows (§7.4), and link classification at composition — see 6.2a, which this step, Step 6.6 (editing), Step 7.1 (comments), Step 8.1 (extended bio) and Step 9.1 (card items) all call. Also **`POST_MIN_INTERVAL_MINUTES` spacing** (SPEC §7.3, §13.6): compare against the author's last feed post's timestamp — not a daily counter — and refuse with "you can post again in N minutes." Profile posts are exempt.

**6.2a [AI] The link classifier (SPEC §7.2.3, §7.2.4; ARCHITECTURE §7).** **One shared function**, called by every surface that accepts text (feed posts, profile posts, comments, the extended bio, contact-card items) — never reimplemented per surface. It returns **one of three answers**, not pass/fail:

- **`REFUSED`** — the host is on the **blocklist** (checked *first*, so no later rule can produce a clickable blocklisted link). The save fails.
- **`CLICKABLE`** — `https`, host matches **exactly or as a subdomain** of an active allowlist row, the row's **surface scope** includes the calling surface, and the URL trips none of that row's **redirector patterns**.
- **`PLAIN`** — everything else. Rendered as a **copy box**, not an error.

The classifier takes the surface (`post` | `card`) as an argument, because allowlist rows are scoped: **messenger domains are card-only**, so a WhatsApp link is a hyperlink on a contact card and a copy box in a post. The redirector step is the one an implementer is most likely to skip and the one that matters most: `youtube.com/redirect?q=…` and `google.com/url?q=…` pass any host check while handing the reader to an arbitrary attacker page. Belt-and-braces: any otherwise-allowed URL whose query string contains an absolute `http(s)://` URL is downgraded to `PLAIN`. Shorteners are never allowlistable.

**Two rules about the copy — they are requirements, not polish.** (1) **`PLAIN` is not an error.** No message, no warning, no error styling, no note beside the link. A composer that flags an ordinary address as a problem is the wrong build, and it is the likeliest way this step goes wrong, because `PLAIN` is what used to be the rejection (SPEC §7.2.4). (2) **The only refusal message** is the blocklist one: it says plainly that this address cannot be posted here and that the fix is to **remove it** (§16.3). It names **no appeal and no request form** — SPEC §7.2.3 records why. The §13.5 form is for proposing an *allowlist* domain and is pointed at from help, not from this error.

**Rendering.** Allowlisted links render as clickable links, never embedded or preview-carded (§7.2). A `PLAIN` URL renders through the **single link-rendering helper** (ARCHITECTURE §4): the address as text in its own container, wrapped **break-anywhere** so a long URL cannot make the page scroll sideways at 320 px (§16.3 — this must not become a second reflow exception beside the preformatted post), plus a real `<button>` with a **distinct accessible name naming its address**, keyboard-operable, confirming the copy in a **polite live region** (Step 2.5's `_status.html`). With script unavailable the address is still fully present and selectable.

✅ Attack cases, as tests, not a manual click-through. Note that several of these **changed direction in 1.29** — what used to be refused is now rendered plain, and asserting "refused" here would re-implement the old policy:
- `https://youtube.com/redirect?q=https://evil.example` → **`PLAIN`** (not clickable, and *not* an error)
- `https://www.google.com/url?q=https://evil.example` → **`PLAIN`**
- `https://evil-youtube.com/watch?v=1` → **`PLAIN`**; `https://youtube.com.attacker.net/watch?v=1` → **`PLAIN`**
- `http://` (not https) on an allowed host → **`PLAIN`**
- `https://maps.google.com/…` in a post → **`CLICKABLE`**
- a **messenger** domain in a post → **`PLAIN`**; the same URL as a contact-card item → **`CLICKABLE`** (the surface-scope test; a classifier that ignores its surface argument passes every other case here)
- a **blocklisted** host → **`REFUSED`**, with a message that names no appeal
- **Assert the absence of an error** on every `PLAIN` case: no form error, no message text in the rendered composer response. This is the one assertion that catches "built as a rejection with nicer wording."
- Render a 400-character URL at a 320 px viewport → the page does not scroll horizontally, and the copy control has a name that distinguishes it from a second copy control on the same page.
...
- **Validation is shared with creation, not duplicated.** The edit path runs the same link **classifier** (6.2a), length caps, whitespace normalization, hashtag-vocabulary check and image re-encode as the create path — which in practice means content validation lives in the shared model/form layer, never in the create view. **As of 1.29 this carries two controls, and the stakes went up:** a create-only path used to mean a non-allowlisted link could be edited in after publication, which was bad but never produced a clickable link. It now means a **blocklisted** domain can be edited in — the one outcome the platform actually forbids (SPEC §7.8 invariant 4, §7.2.3).
...
✅ **Written as attack cases, not a click-through:** publish a post with clean text, then edit a **blocklisted** URL into it → **refused** (this is the whole reason this step exists — a classifier wired only into the create path is the defect being prevented, and it fails silently); edit a non-allowlisted URL into the same post → **accepted, rendered as a copy box, with no error** (the 1.29 direction change: this case used to be a refusal, and asserting a refusal here rebuilds the old policy); edit a post and confirm it does *not* move in a second account's feed; backdate a post 89 days, edit it, confirm it still expires on schedule rather than getting another 90 days; confirm no notification fires to anyone on the edit; as the post author, confirm there is no route to edit someone else's comment.
...
✅ *Phase milestone (two browser windows):* post to a hand-picked audience — the included friend sees it, the excluded one doesn't; unfriend and confirm old posts vanish; try a non-allowlisted URL — it posts, as a copy box, with no complaint; try a blocklisted one — refused before publishing, with a message naming no appeal; backdate a seeded post 80 days — countdown appears; run `expire_content` against a backdated-91-days post — gone, image file too.
...
- `short_bio` ≤ `BIO_SHORT_MAX` — basic tier, so **FoFs see it and it rides along in every friend-request card** (§5.2). Screened against `name_blocklist` at every save (the same check as display names, Step 2.3's neighbour), **rendered with links disabled** — URLs in it are inert text, never linkified, not even allowlisted ones — **and disallowed URLs are rejected at save, not merely de-linkified** (SPEC §9.4). This is the one surface that keeps the pre-1.29 strict rule, and the reason is in the step so nobody "fixes" it into consistency with the rest: **the short bio is pushed at up to 20 people a day who never asked for it** (§5.2, §13.6). A post is not.
- `extended_bio` ≤ `BIO_EXTENDED_MAX` — friends only, so it is not a push surface: it follows the **ordinary three-outcome rule via 6.2a** (allowlisted → clickable, anything else → copy box, blocklisted → refused), no cooldown.
...
✅ Set a short bio, immediately fix a typo (allowed, inside the grace window), then try a third change an hour later → refused with an honest "you can change this again in N hours"; clear it to empty → allowed immediately; paste a **non-allowlisted** URL into the short bio → **rejected at save** with an honest message (not merely de-linkified — SPEC §9.4); paste an allowlisted URL into the short bio → saved, rendered as inert text with no link; paste an allowlisted URL into the extended bio → renders as a link; paste a non-allowlisted URL into the extended bio → **saved, rendered as a copy box, no error** (the extended bio follows the post rule, the short bio does not — this pair of cases is the whole point).
...
**9.1 [AI] Card + cascade.** ≤`CONTACT_ITEMS_MAX` items of kind **phone / email / link**; default → group override → individual override, deny-beats-allow between groups (SPEC §10.2–10.3) — resolution lives in the engine and the tables were defined in Step 4.1 (already tested in 4.2; this step is UI only).

**Two things arrived in 1.29 (SPEC §10.2), and the gate on the second lifted in 1.30.**
- **Every item carries a `label`** ≤ `CARD_ITEM_LABEL_MAX` — *"My photos"*, *"Work mobile"* — **screened against `name_blocklist` at every save**, exactly as `short_bio` is (Step 8.1). The label is never linkified.
- **A `link` item may carry any URL, classified through 6.2a with surface `card`**: clickable if the row is allowlisted **with card scope** (messenger domains today), a copy box otherwise, refused if blocklisted. Do **not** store clickability on the item — it is read from the allowlist at render time, so an operator re-scoping a row changes every existing card at once (ARCHITECTURE §4).
- **GATE LIFTED in 1.30 (it carried a 🚧 through 1.29) — build all three kinds, `link` included.** The gate said link items must not ship while a card had no report action, which was the gap SPEC §13.2 closed in v1.16 for the friend-request card. SPEC v1.30 closes it here: a card item is a report target (SPEC §13.2), the action lives on the card page built in Step 9.2, and the report machinery itself is Step 13.1 with every other report. **Nothing in this step is conditional any more.** Build it whole; the report buttons it makes necessary are rendered in Step 9.2 and wired in Step 13.1, so they are inert until Phase 13 — exactly as the profile's report action is inert between Step 8.1 and Step 13.1.

**9.2 [AI] Card requests, and the card page (rewritten 1.30).** Picker-based request; the system auto-replies by opening the requester's **card page** (SPEC §10.4). Four rules, and the first is the one everything else follows from:
- **The page is resolved live on every visit and nothing is stored.** It calls the engine's `visible_contact_card(viewer, owner)` on each render, exactly as the request's own auto-reply does (ARCHITECTURE §5.1). **Do not store the answered card** anywhere — not on `card_requests`, not in a notification, not in a session. A stored answer keeps showing an item the owner has switched off, which is the one direction SPEC §10.3 says must never fail, and ARCHITECTURE §4 records that this table's earlier wording ("what was auto-answered") read as an instruction to store it.
- **One page per (owner, viewer) pair**, at its own UUID address, permission-checked like everything else (SPEC §9.3), reachable afterwards from the owner's About tab (SPEC §9.1) for as long as the friendship lasts. It carries the owner's display name through the shared name helper, the items, and nothing else. **It is a new page, so it joins the Step 2.5 template smoke-test page list** (Appendix rule 9).
- **A return visit is not a new request**: no `card_requests` row, no notification (SPEC §12.1), no rate-limit counter (SPEC §13.6). The obvious implementation — treat every view as a request — turns a friend's second look at a phone number into an event in the owner's feed.
- **The per-item report action is rendered here and wired in Step 13.1** (SPEC §13.2). A real `<button>` with visible text, one per item, each with a **distinct accessible name taken from the item's label** — *"Report the item labelled 'My photos'"* — falling back to kind and position where the label is empty (SPEC §16.3's repeated-controls rule; Appendix rule 9). **No action on an empty card.** Not shown to the owner; present but inert in preview-as (Step 9.3), which is read-only by construction.

**9.3 [AI] Preview-as for the card** (§9.5).
✅ *Milestone:* build a card with a phone number off-by-default, on for group "Close", off individually for Bob who is in "Close" — Bob's auto-reply lacks the number (individual wins); a friend in no group gets defaults only; preview-as agrees with reality; a label containing a `name_blocklist` string is refused at save.
✅ *Links on a card (the gate lifted in 1.30):* a messenger URL as a card item → **clickable**; the same URL in a post → **copy box** (the surface-scope pair from 6.2a, asserted end-to-end); an arbitrary photo-album URL as a card item → copy box, no error; a blocklisted URL as a card item → refused.
✅ *The card page is live, and this is the test that proves it:* Bob opens his card page and sees the number; the owner switches that item off for Bob; **Bob reloads and the number is gone** — with no new notification to the owner for the reload, and no new `card_requests` row. A test asserting the *absence* of both is the one that catches "every view is a request."
...
**The contact-card target type (new in 1.30, SPEC §13.2).** A fourth target beside post, comment and profile, in the same queue, on the same form. It is the one whose frozen copy is not a copy of a stored row, so build it deliberately:
- **The freeze is one call to `visible_contact_card(reporter, owner)` at submit**, with the **reporter** as the viewer (ARCHITECTURE §5.1). **Never resolve the cascade in the report code** — that captures every item the owner has, including ones this reporter was never shown, and files them as evidence. This is the failure to write a test for, not just to avoid: seed an owner with an item hidden from the reporter by an individual override, file a report, and **assert the hidden item is absent from the frozen copy.**
- **Store it as a value**: an ordered list of `{kind, value, label}` plus the **position** of the reported item. No foreign key to `contact_items` (the owner deletes that row first), no group ids, no override rows, no clickability flag (re-derived from `url_allowlist` at view time, Step 6.2a).
- **Target categories** (SPEC §13.2): *the label · the address or number · the card as a whole · this person's behaviour*, plus the same optional note to the operator every report carries.
- **Purge is unchanged** — dismissed → now, upheld → +30d, hard cap 90d, via `purge_moderation`. No new branch: the frozen card is text, so there is no file to delete.
- **Wire the buttons rendered in Step 9.2.** The action, its placement and its accessible names belong to that step; this step gives them a form and a queue row.
...
**13.3 [AI] Django admin as operator console.** Moderation queue with act-on actions (delete content / warn / ban), vocabulary editor, `NAME_BLOCKLIST` editor (SPEC §4.5), `REACTION_SET` and `THEME_SET` editors; admin at a non-default path (ARCHITECTURE §7). **URL-allowlist editor** (SPEC §7.2.3, ARCHITECTURE §4): each row carries host, **admitting category** (convening / hosts-what-we-cannot-host / messenger handoff), and **redirector-rejection patterns**; rows are **created inactive by default**, so activating one is the deliberate moment the operator confirms the service has no open redirector. Seed the known cases (`youtube.com/redirect`, `google.com/url`). Each row also carries a **surface scope** — posts-and-comments / cards / both (SPEC §7.2.3, new in 1.29); seed messenger domains as **cards** and everything else as posts-and-comments. Deactivating beats deleting — it keeps the record of why a domain was added.
**URL-blocklist editor** (SPEC §7.2.3, ARCHITECTURE §4, new in 1.29): a **second table beside the allowlist**, host plus an active flag, same deactivate-never-delete discipline. It needs no scope and no category — a blocklisted domain is refused everywhere. Seed it empty; what belongs on it is operational judgment, not a design decision. Two things belong in the admin copy: the blocklist is read by the **same classifier** as the allowlist on **every save path** (Step 6.2a, Step 6.6), and there is **no appeal channel** for it (SPEC §13.5), so no operator workflow is needed for one.
**Contact-card reports in the queue (new in 1.30).** A card report renders its frozen copy as **the delivered items in order, with the reported one marked** — labels and values as text, nothing resolved and nothing linkified (SPEC §13.2). **Delete content on this target removes the whole `contact_items` row** the report names (SPEC §13.2.1): never a partial edit, never a blanked label, and never anything on `contact_overrides`, which is the owner's configuration rather than content. Where the row is already gone the outcome is unavailable and the report can still be upheld — the §13.4 counter is the record. Where it exists but has changed, show the operator **both** the frozen copy and the current row and let them choose; removing it is still correct, and dismissal is what a fix is for.
...
✅ *Milestone:* report a seeded post; see the frozen copy in admin; dismiss → copy purged; uphold another → live content gone, counter incremented, copy purged after the (test-shortened) window; a script-speed action burst hits the limit politely. **Then the card case end to end:** Bob reports an item on Alice's card; **Alice deletes that item immediately**; the operator still sees exactly what Bob received, and an item Alice had hidden from Bob is **not** in the frozen copy; uphold → the named row goes from every version of Alice's card at once, and nobody is notified.
...
**16.1 [AI] Security settings pass.** The Django production checklist (`manage.py check --deploy` clean), cookie flags, header settings, upload size limits — ARCHITECTURE §7. **Confirm the auth-security layer (SPEC §4.6.1):** Argon2id is the active password hasher, reset/verification use codes with no login link anywhere, login backoff is enforced, and the breach-password check runs at registration and password change. **Confirm the link classifier (SPEC §7.2.3, §7.2.4):** the Step 6.2a cases still pass against the live allowlist **and blocklist**, every surface that accepts text routes through the one shared function **with its surface argument** (a card item and a post must disagree about a messenger domain), and no active allowlist row has an unchecked redirector. **Confirm the classifier runs on the edit path too** (SPEC §7.8 invariant 4, Step 6.6): re-run the cases as *edits* to already-published clean content, not just as new posts — a **blocklisted** host edited into a clean post must be refused. A create-only classifier is the single most likely way this control gets defeated in practice, and it fails silently — nothing errors, the link simply goes live. **Confirm also that `PLAIN` is still silent:** a non-allowlisted URL posts with no error, no warning and no error styling on every surface, which is the regression a later "tightening" pass is most likely to introduce.
...
10. **Every content save re-runs every content check** (SPEC §7.8 invariant 4). Validation belongs to the shared model/form layer that both the create and the edit path call — never to a create view. A check that runs only on creation is a defective implementation, and with the **link classifier** that defect is a security hole rather than a bug: it fails silently, with nothing to notice. **Two controls now ride on this rule** (1.29): the allowlist decides what becomes clickable, and the **blocklist** decides what is refused outright — the second being the one a create-only path lets an author edit past.
11. **Five things are rendered by one shared helper each, never reimplemented** (ARCHITECTURE §4): display names, relative timestamps, image alt text, theme selection, and — since 1.29 — **a URL in stored text**, as a hyperlink or a copy box, from the §7 classifier's answer (SPEC §7.2.4). *(The count was left at four when the fifth helper was added; corrected in 1.30.)* The time helper emits the phrase **only** — no `title` attribute, no `<time datetime="…">`, no data attribute carrying the true timestamp (SPEC §7.5.1). If a surface seems to need a date formatted some other way, stop and say so; that is a design conversation, not a coding decision.
