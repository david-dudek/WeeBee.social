...
**This file last changed in:** 1.29 (Steps 6.2a, 6.6, 8.1, 9.1, 13.3, 16.1 follow SPEC's link policy)
...
**Two things are new in 1.29 (SPEC §10.2), and one of them is a gate.**
...
- 🚧 **GATE — link items do not ship until the card-report path exists.** A contact card is not a reportable object anywhere in SPEC (§13.2 covers posts, comments and profiles), and it must not become a surface carrying an author-chosen address and an author-written label with no way for the recipient to report it — the same gap §13.2 closed in v1.16 for the friend-request card. The design is `prompts/15-reporting-a-contact-card.md`, not yet run. **Until it lands, build the phone and email kinds and the label field; leave the `link` kind out of the UI.** The hard part is that a card is per-requester (§10.3), so the operator needs the version the *reporter received*, not the owner's current rows.
...
✅ *When the gate above lifts:* a messenger URL as a card item → **clickable**; the same URL in a post → **copy box** (the surface-scope pair from 6.2a, asserted end-to-end); an arbitrary photo-album URL as a card item → copy box, no error; a blocklisted URL as a card item → refused.
