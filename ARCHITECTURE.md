...
**This file last changed in:** 1.22 (§3.8 gains the operator-console boundary; §9 puts the two accessibility tests on a continuous footing, gives the template smoke tests a build home, and records the axe/`pa11y` decision; §15 item 7)
...
- **hashtag_vocab** — the operator-curated vocabulary (§11.2), edited in Django admin. Each row also carries **search aliases** (SPEC §11.2.1) — the synonyms a user might type for that interest. They are matched by the picker's search and **never rendered, never selectable, and never joined to `profile_hashtags` or `post_hashtags`**: an alias is a route to a tag, not a tag. Storing them on the row rather than in a table of their own is deliberate — they have no identity apart from their tag, and nothing ever queries them except that one search.
