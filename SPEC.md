...
**This file last changed in:** 1.25 (reactions gain a lifetime, a rendering specification and curation rules — new §8.2.1–§8.2.3, with §7.5, §7.6, §9.7, §12.1–§12.3, §14 and §16.3 updated to name them)
...
- **A reaction is visible to exactly one person: the author of the thing it is attached to.** It is shown to them as names, never as numbers ("Alice: Love it! · Mom: So proud!"). Every other viewer sees nothing — no counts, no indicators, no sign that any reaction exists. This is deliberate: warmth without scorekeeping; no validation economy.
