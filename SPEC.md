...
- **Not a walled garden.** The platform does not try to own a relationship. Where a need is already well served by open, established channels — person-to-person messaging above all (§10.1) — WeeBee delegates rather than rebuilds, and makes the handoff cleanly. It aims to be one useful tool among many, never the place a user cannot leave (see also credible exit, §4.9). **This bullet is the summary; the rule, its test and its limits are §1.5.**
...
### 1.5 The Delegation Principle (v1.29)

**The principle, in the founder's words:**

> "One of the design principles that I used when coming up with WeeBee is that I didn't want it to try to add features that are better implemented elsewhere. WeeBee is meant to be used in concert with other services like texting, email, Google Maps, Evite, DropBox, etc. Instead, links to other services that are approved can go on posts, and any other link can go on contact cards."

**The Delegation Test (apply to every current and future feature):** *Is this need already well served by a channel the user has? If so, WeeBee's job is the handoff, not the capability.*

**It relocates; it does not refuse — and that is its difference from both other tests in this section.** §1.2 is a **gate**: a feature that fails the No-Reach Test is wrong for this platform and there is nothing further to discuss. §1.4 is a **question**: a feature that fails the Gathering Test may still be built, provided the reason is written down beside it. This test is neither, because it is not asking whether the *need* belongs on WeeBee — it asks **who should be serving it**. A feature that fails it is not refused; it is **moved outside, and WeeBee carries the link to it**. The answer to "WeeBee doesn't do X" is therefore almost never "so nobody can": it is *"so do X where X is done well, and say so here."* **A refusal that does not name the door out is an incomplete application of this principle**, in this document and in user-facing copy alike.

**Delegation is not abdication — the courtesy minimum.** Where a capability is what makes the platform usable at all, WeeBee provides its own meager version and delegates the rest. **Photographs are the case that defines the rule** (§7.2.3, in the founder's words): what WeeBee can offer is slight next to any real photo service, and it has to offer it anyway, because a social platform where a user cannot post a single picture without an account somewhere else will never get off the ground. So: **one photograph per post, `GALLERY_MAX` = 8 on a profile** — enough that the platform works on its own, never enough to be anyone's photo library. The limit is the delegation, and the link is where the other fifty-two pictures live.

The line this draws is a judgment made feature by feature and written down, not a formula: *what is the least this platform must do itself for the thing it is actually for to work?* Below that line WeeBee builds. Above it, WeeBee links.

**What is delegated today, and to what.** Person-to-person conversation, to the messengers and mail people already use (§10.1) — the largest instance, and the reason there are no DMs. Events, invitations and RSVPs, to the services built for them (§17). Video and audio, hosted nowhere here (§7.2). Photo libraries and file storage, per the courtesy minimum above. Maps and venue pages, to the map services. In every case the platform's contribution is the part that is genuinely *not* solved elsewhere: a bounded audience of vouched friends, and a handoff to the channel that does the rest.

**What is never delegable, stated so the test cannot be read backwards.** The friend graph, the audience model, the visibility rules, expiry, and accessibility are what WeeBee *is*; that other platforms do adjacent things better is not an argument about any of them. The test asks whether **a need the user brought with them** is already served on a channel they already have — not whether some company somewhere runs a better version of this platform. Read the other way it becomes an argument for building nothing, which is the failure mode this section is most likely to be quoted for.

**Nor does it license making the platform worse in the name of purity.** The courtesy minimum above is the limit on that, and it is the half of this principle most easily lost: *"that's better elsewhere"* is a complete argument for **not building a photo service** and never a complete argument for **refusing to hold a photo**.

**Why this needed a name.** The principle has governed this project from the start and appears throughout it — §10.1's *"a platform that rebuilds messaging is not adding a capability, it is trying to own a relationship it did not create"* is its best statement anywhere — but until now it existed only as the local justification for individual refusals, and each refusal re-argued it. §1.3's **"not a walled garden"** bullet is the same principle in one sentence with messaging as its only example, which reads as a footnote to §10.1 rather than as a rule the rest of the document cites; README did not mention it at all, and described the platform almost entirely by its absences, with nothing to say those absences are deliberate handoffs. §1.2 shows what the difference costs: a principle with a named test gets cited and inherited, and a principle without one gets re-derived or forgotten. The §1.3 bullet stands and is not deleted — it is the summary; this section is the rule.
...
- **First interface:** A simple, mobile-friendly **web application**. No native mobile app in v1. No public (logged-out) pages except login/registration/invite-acceptance. **This is a rule about *pages*, and the boundary was settled on 2026-08-18:** the unauthenticated health endpoint of ARCHITECTURE §7.3 returns the literal body `ok` and nothing else — no HTML, no template, no theme, and nothing about any user — so it is not a page and this list does not name it. The reasoning belongs to ARCHITECTURE §7.3, which owns the endpoint; it is pointed at from here so that a reader of this line need not reconstruct it, and so that the next unauthenticated route proposed has a test to meet rather than a precedent to cite.
...
**Why most phishing vectors are already closed — narrowed to what is actually true (v1.29).** The platform gives an attacker almost no in-platform way to deliver a *clickable* deceptive link, and **no way at all to deliver one unsolicited**: no direct messages (§10.1); no free text in friend requests, introductions, or reactions; no public registration (§4.1) — an unsolicited "click to join" is inherently suspicious; and the URL allowlist, which decides what becomes a hyperlink on every post, comment and contact card (§7.2, §7.2.4).

**What v1.29 changed in this claim, and what it did not.** Until v1.29 a non-allowlisted URL could not be *delivered* here at all; now it can, as inert text in a copy box. The claim is therefore smaller and is stated in its smaller form above. Three things keep the argument standing:

- **The unsolicited half is untouched, and it was always the larger half.** Every surface that carries a link is one a reader came to — a post from a friend they chose to follow into their feed, a profile they visited, a card they requested. Nothing pushes an address at anyone. Phishing at scale depends on unsolicited delivery, and the structures that refuse it here (no DMs, no composed text in requests or introductions, no public signup) are unchanged.
- **A copy box is not a weaker rendering of a hyperlink for this purpose.** Posts are plain text (§7.2) with no markup, so a URL is linkified as itself and link text can never lie about its destination — on either path the reader sees the true address. What the copy box removes is the one-tap reflex: reaching a hostile destination now takes a deliberate copy and paste, and the address arrives in the reader's own address bar where they see it again.
- **A blocklisted domain is refused outright** (§7.2.3), which is a control this layer did not have before at all.

The realistic remaining vector is still **email**, the one channel where the platform legitimately sends links (invites, verification, recovery, inactivity notices).
...
- **An outstanding invitation cannot be redeemed while the account that sent it is invisible (v1.27).** Unredeemed invites are voided on the ordinary expiry path of §4.1 and return to the sender's budget, where they simply wait. The reason is not punitive and is not about deletion at all: §4.1 auto-friends inviter and invitee, so an invite completing here would produce a brand-new member whose single friend is invisible — no friend-of-friend path to anyone, nobody who can see them, no way to be found. **This is a property of invisibility, not of any one account state**, which is why it is stated once here: the other state that has it is a ban (§13.2.1), and a ban inherits this rule rather than restating it.
...
- **The two deletion warnings state the absolute deletion date (v1.26)** — *"your account will be deleted on 12 March"*, never "in a couple of months". An impending deletion is an account event, and therefore one of §7.5.1's two narrow exceptions to relative time, on the precedent §4.7 already sets with the deactivation banner's absolute erasure date. This is why §12.3's email rule was narrowed to *relative* ages in v1.26: under the older "email carries no timestamp at all" these warnings could not say **when**, and a deletion warning that cannot say when is not a warning.
- **A banned account is swept on the same clock, with two adjustments (v1.27, §13.2.1).** A ban runs no clock of its own; left alone, a banned account's content expires at 90 days and the account itself is deleted here. Because a banned person can still sign in to export or delete their data (§13.2.1), **that sign-in does not refresh the last-login date** — otherwise the sweep could be postponed indefinitely by reading the suspension notice. And of the four warning emails, a banned account receives **the last two and not the first two**: a dormancy note invites a person to come back, which is exactly wrong here, while a deletion warning is a notice that data is about to be destroyed, which a moderation outcome does not cancel.
...
**One composer for both types (v1.16).** There is a single composer, reached from the main navigation — not a separate composer on the profile page, which would make the destination implicit in where the author happened to be standing rather than something they chose. One composer is also the only way the shared rules stay shared: length caps, whitespace normalization, the preformatted toggle, the link policy of §7.2.3–§7.2.4 including its open-redirector check and the blocklist, the image-plus-alt-text prompt, and the hashtag picker are all written in this document as rules for *both* types (§7.2, §7.2.1, §7.8), and two implementations are how §7.8's security-critical invariant 4 quietly stops running on one path.
...
- **Links follow one uniform rule on every surface (v1.29).** A URL from an **approved** service renders as a clickable hyperlink; **any other** URL renders as an inert **copy box** showing the address in full; a URL on the **blocklist** is refused, and the post cannot publish. The two lists are maintained operational artifacts, never user-configurable. **What each list admits and why — including the mandatory open-redirector rule — is §7.2.3; what a reader sees in each of the three cases, and the copy that goes with it, is §7.2.4.** The same three outcomes govern posts, comments and contact-card items (§10.2). The profile's bio fields carry one rider (§9.4): the FoF-visible **short bio renders no links at all and refuses every disallowed URL at save**, because it is pushed toward people who did not ask for it; the friends-only extended bio follows the uniform rule above.
- Images: EXIF metadata (including GPS location) is **stripped on upload, always**. Uploads are re-encoded/resized server-side.
- No video or audio **hosting**, no polls. Links to allowlisted media services (YouTube, SoundCloud, etc.) are permitted **as clickable links only** — never auto-embedded as players or preview cards, because third-party embeds load the external service's scripts and trackers into the page, violating the tracking ban (§15.2). (Parked: privacy-safe "click-to-load" embeds — a placeholder that connects to the external service only after the viewer explicitly clicks it.)
...
### 7.2.3 The two link lists: what they admit and why (v1.12, rewritten v1.29)

There are **two operator-curated lists**, and they do different jobs. The **allowlist** decides what becomes a **clickable link**. The **blocklist** decides what is **refused outright**. Everything on neither list is delivered as an inert copy box (§7.2.4). Until v1.29 there was one list and it governed *delivery*; it now governs *clickability*, and §7.2.4 records what that trade cost and bought.

**The allowlist does two jobs at once, and both are deliberate.** The first is **anti-phishing**: it is the control §4.6.1 leans on, in the narrowed form §4.6.1 now states — there is no way to deliver a *clickable* deceptive link. The second is to **state what the platform is for** — WeeBee carries the links that serve the mission of §1.1 and hands everything else to the channels of §1.5. The list is a purpose filter, not merely a security whitelist that happens to contain map services.
...
2. **Hosting what WeeBee cannot host, or cannot host at a useful size (amended v1.29).** Two cases, and the honest difference between them matters:
   - **What WeeBee hosts none of.** Video and audio (§7.2, §17) — out of scope for a solo operator — so YouTube, Vimeo, SoundCloud, Bandcamp and equivalents are where a user's **own** recording lives.
   - **What WeeBee hosts a courtesy minimum of.** Photographs. WeeBee holds **one image per post** and `GALLERY_MAX` = 8 on a profile, and photo-hosting services are admitted for everything past that. This is not "we can't host it"; it is the courtesy-minimum rule of §1.5, in the founder's words: *"I think we could add photo hosting because what we can offer for that is meager. Even though there are better photo hosting services, we still have to provide what we do, because this will never get off the ground if users can't even post one photo."* The prior wording scoped this category as *what WeeBee cannot host* and justified it with *"WeeBee hosts no video or audio,"* which excluded photo services by its own logic — WeeBee plainly does host images. The category is the shape of the limit, not the absence of the capability.
   - The stated purpose in composer and help copy stays exactly that: *"WeeBee doesn't host video or audio. This is where your own recording lives."* And for images: *"WeeBee holds one photo per post. If you have sixty, they live somewhere else — link to them here."*
3. **Handoff — official messenger link domains** (WhatsApp, Signal, Telegram and equivalents). These move a conversation to a channel WeeBee deliberately does not provide (§10.1) — §1.5 made concrete. **These are card-only as of v1.29** (below); they are the first rows to be.

**Every allowlist row carries a surface scope (v1.29): posts and comments, contact cards, or both.** A row says not only *is this clickable* but *where*. **Messenger domains are card-only**, which is a change: a WhatsApp link in a post renders as a copy box like any other unapproved address, and becomes a hyperlink only as an item on a contact card (§10.2). Maps, ticketing, media and photo hosts remain posts-and-comments; nothing else is card-only today.

**The guideline that decides the scope — offered as a guideline, not a rule (founder, v1.29).** *A link that identifies a **person** belongs on the card; a link that identifies a **thing** belongs in a post.* A messenger handle is you. A map pin for a restaurant is a thing. It is written down because it gives a principled first answer to the cases that will actually arrive — payment handles, scheduling links, and professional-profile services such as LinkedIn, which the founder names as the likely second card-only category — rather than deciding each from scratch. It is **explicitly not a test in the sense of §1.2, §1.4 or §1.5**: it settles nothing on its own, and the operator may scope a row against it with a reason.

**Original content: an aspiration, honestly not an enforceable rule.** The founding preference is that what people post here is their own, and category 2 largely serves it — the link is a prosthetic for a capability the platform lacks or deliberately bounds, not a channel for someone else's work. But **a link to one's own upload and a link to someone else's are identical at the URL level.** The difference is intent, and no domain rule can read intent. The platform therefore states the preference in its copy and does not pretend to enforce it.
...
**The anti-phishing bar (every candidate, every category, every scope).**
...
**The blocklist (v1.29).** A second operator-curated list of domains that may not be delivered here **in any form** — not as a link, not as text. It exists because §7.2.4 made every address deliverable, and a small number of destinations should not be, whatever the delivery mechanism. Its mechanics:

- **It is a table, maintained by the operator in the console alongside the allowlist** (ARCHITECTURE §4, BUILD_PLAN Step 13.3), never a constant in code. It inherits the allowlist's **"deactivating beats deleting"** discipline: a row is withdrawn by clearing its active flag, so the record of why it was added survives — the same discipline `NAME_BLOCKLIST` (§4.5) and `REACTION_SET` (§8.2.3) already follow.
- **It is checked by the same shared validator as the allowlist, on every save path, create and edit alike** (§7.8 invariant 4, ARCHITECTURE §7, BUILD_PLAN Steps 6.2a and 6.6). This is the second control to depend on that invariant, and it fails the same silent way: a validator wired only into the create path lets an author publish clean text and edit a blocklisted link into it afterwards. **A create-only implementation is a defective implementation of this section**, exactly as §7.8 says of the first control.
- **What belongs on it is not specified here** and is not a design question — it is operational judgment exercised case by case, in the same way `NAME_BLOCKLIST`'s contents are. What is specified is that the mechanism exists, where it is checked, and what the author is told (§7.2.4).
- **There is no appeal channel, and that is a decision rather than an omission.** §13.5's operator request channel accepts a proposal to **add a domain to the allowlist** — a judgment about purpose, which the three categories above make arguable. It does **not** accept a request to remove a domain from the blocklist. §13.5 sends no replies at all by design, so an appeal route would be a door that never opens, which is worse than naming no door; and the honest fix for a blocklisted link, per §16.3's requirement that an error state its fix, is to remove the link. §7.2.4 carries the wording, and it promises nothing it cannot do.

### 7.2.4 What happens to a link: three outcomes (v1.29)
...
**Only the third is an error, and the difference is the whole of the user experience here.** A copy box is **a different rendering, not a refusal**: no warning, no apology, no error styling, no "this link is not approved" note beside it. The author is not told off, and nothing in the composer flags the address as a problem, because it is not one. This is a rule about copy and styling, and it is load-bearing: on a platform that already says no to a great deal, a system that also scolds the user each time is experienced as scolding them generally, and the person who feels scolded is the ordinary member posting a link to their own photo album.

**The copy box, specified.** It shows the URL **in full and verbatim** — never truncated in the value itself, never re-written, never resolved. Alongside it sits a control that copies the address to the clipboard. Its obligations are §16 requirements, not polish:

- The control is a real `<button>` with a **distinct accessible name** naming its link, not a bare icon (§16.3, §16.4) — a page can carry many, and an element list reading "Copy" twelve times is a dead end. This is the same rule the "read more" folds and the reaction pickers already follow.
- It is **operable by keyboard alone** (2.1.1), and the copy is **confirmed in a polite live region** (4.1.3), composing the existing status partial — a control whose only feedback is a visual flash tells a screen-reader user nothing.
- **The address must not break the 320 px reflow guarantee** (§16.3, 1.4.10). URLs do not wrap at ordinary break opportunities, and a long one is exactly the two-dimensional-scroll failure §16.3 forbids. It is solved here, not inherited: the address wraps **break-anywhere inside its own container**, and where a design prefers truncation the **full address must still be what the copy control copies** and must still be reachable as text. **This does not create a second reflow exception.** §7.2.1's preformatted post remains the platform's single documented exception, and a copy box that quietly acquired a horizontal scrollbar would be a second one arriving by accident, which is a bad trade for a control that appears on ordinary posts.

**Why a copy box rather than bare text — founder decision, 2026-09-07.** Both options disclose the destination, and on this platform so does a hyperlink: posts are plain text (§7.2), a URL is linkified as itself, and there is no markup with which link text could ever lie about where it goes. So the copy box's gain over a hyperlink is **not disclosure — it is the deliberate act**: no accidental tap, no one-tap phish, and on a phone an app-switch the reader has to mean. Its cost against bare text is that it is the weaker of the two against the drift the friction exists to prevent, which the founder states as *enough friction to keep WeeBee from devolving into link sharing*. Bare text was rejected as needlessly punishing: it costs a fiddly select-and-drag or retyping thirty characters, and it falls hardest on the ordinary use — a person linking to the rest of their own photographs.

**One honest limitation, stated once, here, where the choice is recorded.** A platform with no counters (§1.2) **cannot measure whether a control on content shape is working.** There is no metric that will tell the founder in three years whether feeds have drifted toward link-passing; there is only his own reading of them and the report queue (§13.2). That is the design working as intended rather than a flaw — the absence of measurement is the point of §1.2 — but the consequence is real and belongs beside the decision: **this choice cannot be revisited on evidence later.** What can be said is which direction is cheap. Loosening friction is easy and takes nothing from anyone; tightening it removes something people have got used to. Should the copy box prove too weak, the available move is the stricter rendering, and the reason for making it will be a judgment, not a number.

**The blocklisted refusal.** The author is warned **before the post is published, not after** — the check runs at composition and again at save, on the create and edit paths alike (§7.2.3, §7.8 invariant 4). The message follows §16.3: it says plainly that this address cannot be posted here and that the fix is to remove it. **It names no appeal**, because there is none (§7.2.3), and an error that gestures at a door that does not open is worse than one that does not.

**What this policy costs, stated plainly.** Before v1.29, a URL outside the allowlist could not be delivered in-platform at all. It now can — as inert text that a determined reader can copy. Three things are true about that and all three belong on the record:

- **The anti-phishing claim is genuinely smaller, and §4.6.1 and §13.1 have been narrowed to match.** The honest form is *no way to deliver a clickable deceptive link, and no unsolicited delivery at all* — WeeBee still has no channel that pushes a link at someone who did not ask for one (no DMs, no free text in requests, introductions or reactions), and that, not the allowlist, was always the larger half of the argument.
- **What is bought is that the platform stops refusing ordinary, harmless things,** and that §1.5's promise — *WeeBee limits what it carries, never what you may say, and it always names the door out* — becomes true of the documents rather than aspirational. Under the old rule the door out was named and then bolted: the only surface a user was told to use for "anything else" was the contact card, which permitted no such item (§10.2, corrected in v1.29).
- **Nothing here loosens the no-reach guarantee.** Off-platform re-propagation — screenshots, retyping, chain letters — is outbound leakage, conceded honestly in §1.2 and §17. Link policy is **inbound content shape**, entirely within the platform's control, and conceding the first has never been an argument for loosening the second. This change is made on its own merits, not on "we can't solve every problem on the internet."
...
- **Every post and every comment is permanently deleted `CONTENT_TTL_DAYS` = 90 days after creation, with one exception: posts that are currently pinned (§7.6).** No archive, no soft-delete, no operator copy (sole exception: frozen moderation copies, §13.3). The exception is stated here, and not only in §7.6, because an expiry job written from this bullet alone would delete pinned posts. **Comments have no such exception, and neither do reactions** — a comment on a pinned post expires at its own 90 days regardless of the pin, and so does a reaction (§8.2.1, v1.25).
...
- **What this rule does not govern.** Expiry applies to posts, comments and reactions (§8.2.1). The profile photo, the gallery, both bio fields, profile hashtags, the contact card, groups and the friend list are **account state**, not statements, and do not expire — see §9.7.
- **What the promise is about, now that a post may carry a pointer to something durable (v1.29).** §7.2.4 lets a post carry any address, which invites the fair challenge: if the photographs live at the far end of a link and only the pointer expires, what is the 90-day promise worth? The answer is the one this platform has always given, and it does not change here — **the promise is about what the platform retains and could be compelled to produce**, never about what users choose to keep. It is the same posture §1.2 takes on screenshots and manual re-propagation. A pointer that expires is not a weaker promise than a paragraph that expires: in both cases WeeBee's copy is gone on schedule, and in neither does WeeBee claim any hold over what a person put somewhere else.
- Deletion includes attached images and all comments/reactions on the expired post. Comments and reactions **additionally** die at their own 90 days, whichever comes first (§8.1, §8.2.1).
...
**Two exceptions, both narrow.** The **expiry countdown** (§7.5) shows absolute days remaining. **Account and security events** (§4.6.1) carry absolute timestamps wherever they appear — they are account and security notices, not social content. *Wherever they appear* includes email (§12.3, narrowed in v1.26): the deactivation banner's erasure date (§4.7) and the inactivity warnings' deletion date (§4.8) are this same exception, not additions to it.
...
- **Emails carry no relative age** (§12.3, narrowed in v1.26 from "no timestamp at all"): a relative time is computed when mail is sent and read whenever the recipient opens it, so it would simply be false by then. An **absolute** timestamp does not decay, so the two exceptions above apply in email exactly as they do on a page — which is what this section's own principle already points at, security events and deletion dates both sitting on the precise side of it.
...
- **Reactions on a pinned post expire too (v1.25)**, at their own 90 days, and their absence is likewise unmarked (§8.2.1). The principle below decides this, and decides it the same way: a reaction is another person's statement, attached to them by name, on content its author chose to keep.
- **The governing principle, stated because it will be needed again: an author may preserve their own words indefinitely, and may never preserve anyone else's.** Keeping comments — or reactions (§8.2.1) — alive for as long as a pin would let one person's editorial decision make another person's words permanent inside a 300-person group, without asking. That is the same refusal §7.8 makes when it lets a host delete a comment but never edit one.
...
4. **always re-runs every content validation** — the **whole link policy** (§7.2, §7.2.3, §7.2.4): allowlist membership and surface scope, the open-redirector rule, **and the blocklist** — plus length caps, whitespace normalization (§7.2.1), hashtag vocabulary (§11.2), and EXIF stripping and downscaling on a replaced image (§7.2.2). **This invariant is security-critical, and as of v1.29 two separate controls depend on it.** A save path that validates only on creation would let an author publish clean text and then edit into it either a link the allowlist would not have made clickable, or a **blocklisted domain that would have been refused outright** — the second being the more serious, since it is the one outcome the platform actually forbids. Validating only on create is a defective implementation of this section, exactly as a host-only allowlist check is a defective implementation of §7.2.3.
...
### 8.1 Comments
- Who can comment = who can see the post (§7.4, §11.3). **A comment is visible to the people who can see the post, minus anyone blocked in either direction with the comment's author (§5.4) — never more.** Nothing about commenting propagates content anywhere.
- **The word "exactly" was wrong, and the correction is not cosmetic (v1.26).** Through v1.25 this bullet asserted *equality* — "visible to exactly the people who can see the post" — which §5.4 contradicts: if Bob can see Charlie's post and Alice has blocked Bob, Bob sees the post and not Alice's comment on it. **"Never more" was always true; "exactly" never was.** The comment audience is a **strict subset** of the post audience. The consequence is architectural rather than editorial: because the two audiences were asserted identical, no separate check was ever specified, so every surface rendering a comment either applied the block rule itself or did not apply it at all — and both of those break the rule that one module makes every visibility decision. Comment visibility is now a question the visibility engine answers in its own right (ARCHITECTURE §5). Nothing else in this section changes: who may *write* a comment is still exactly who may see the post, and the consciously accepted consequence below is untouched.
...
- Comments on a post generate a coalesced notification to the post's author and to anyone following it (§12.1, §12.3). **Reactions notify only the author of the item reacted to** — never a follower, who cannot see them (§8.2.1, corrected in v1.25).
...
- A user may attach **one reaction per post or comment**, chosen from a fixed, operator-curated picker of ~6 warm phrases (`REACTION_SET`; e.g., "Agreed!", "Love it!", "So proud!", "Thinking of you", "Congrats!", "Ha!"). Changeable or removable at any time (§8.2.1). No free text, no arbitrary emoji. The phrases are placeholders for operator curation (Appendix A item 8); what a phrase has to pass to be in the set, and what happens to existing reactions when the set changes, is §8.2.3.
- **Who may react = who may see**, exactly as for commenting (§8.1): a person may react to a post or comment they can currently see (§7.4, §11.3), and to nothing else. Reacting widens no audience, creates no access, and tells nobody but the one person reacted to.
- **A reaction is visible to exactly one person: the author of the thing it is attached to** (or, where a block stands between them, to nobody — two bullets below). It is shown to them as names, never as numbers ("Alice: Love it! · Mom: So proud!"). Every other viewer sees nothing — no counts, no indicators, no sign that any reaction exists. This is deliberate: warmth without scorekeeping; no validation economy.
- **On a comment, that one person is the comment's author, and not the post's author (v1.25).** §8.2's own logic settles it — a reaction is for the person reacted to — but the consequence is worth stating, because the other answer is the tempting one: a post author who saw the reactions on every comment on their own post would be handed a column of reaction lines, one per commenter, down a single page. That is a scoreboard assembled out of other people's warmth, and it is the same failure the list-view rule below exists to prevent, arriving by a different door. A host's power over their own post is to **delete** a comment (§8.1); it was never to read the private replies to it.
- **A block makes that one person nobody, and this is the whole of the reaction side of v1.26's correction.** §5.4 says neither party sees the other's comments **or reactions** anywhere, and it does not exempt a reaction already given: if a block later stands between the recipient and the reactor, the reaction leaves the recipient's reaction line at once and drops out of any notification that named its giver — the same live-rendering path §12.2 already applies to a reaction its giver removed (§8.2.1). It is a far smaller correction than §8.1's, and it is not a free one: because a reaction's audience is one person, a block does not narrow that audience, it **empties** it, so the engine renders the line or renders nothing. Like §8.1's, it is a visibility decision and belongs to the visibility engine rather than to the template drawing the line (ARCHITECTURE §5) — and no third party is involved at any point, because no third party can see a reaction at all.
- **Nobody reacts to their own content.** The react control is absent on your own post and your own comment (§8.2.2). A reaction to yourself would put your own name in your own list, and a private tally of self-approval is scorekeeping with a sample of one. Reacting to a *comment on* your own post is ordinary and permitted — the comment is not yours.
...
### 8.2.1 A reaction expires, on its own clock (v1.25)

Until v1.25 this was true only by silence, and silence pointed the wrong way. §7.5 expired posts and comments and said nothing about reactions; §9.7's two lists named neither side; and the expiry job deleted reactions only as a cascade from a deleted parent. **A pinned post is never deleted**, so reactions on one accumulated forever — the single case where the omission had a visible consequence, and the one that forced the question.

- **A reaction is permanently deleted `CONTENT_TTL_DAYS` = 90 days after it was last set, or when the content it is attached to is deleted, whichever comes first.** That is §8.1's rule for comments — a clock of its own, or the parent's death, whichever arrives first — with one difference, in the third bullet below. **Its disappearance leaves no trace** — no tombstone, no "this post had reactions," nothing (§7.6).
- **A pinned post is not an exception, and this is the case the rule exists for.** §7.6 already decided it and did not say so: *an author may preserve their own words indefinitely, and may never preserve anyone else's.* A reaction is another person's statement, attached by name to content its author chose to keep. Two years of them, held alive by one person's editorial decision about their own post, is precisely the preservation of somebody else's words without asking that §7.6 refuses for comments — and the reactor has even less notice than the commenter, having made their statement in two words from a fixed list rather than in a paragraph they wrote.
- **The clock runs from when the reaction was last set.** Changing a reaction is not an edit — there is no version, no diff, and no "edited" marker (§7.8) — it is the retraction of one statement and the making of another, and the new one is new. **This does not contradict §7.8 invariant 2**, which forbids an edit resetting a *post's* clock: invariant 2 protects the 90-day promise about readable content and keeps pinning the only act of preservation on the platform. A reaction is not readable content. It has no audience, cannot be found, linked, followed, ranked or counted, and exactly one person will ever see it. What §7.5 refuses is an accumulating archive; a two-word private note, re-affirmed by the person who wrote it, is not one. **The consequence is stated rather than engineered against:** someone who re-picks a phrase every eighty-nine days keeps it alive indefinitely. That is re-stating, not preserving, and it is a person keeping *their own* words alive — which is the side of §7.6's principle that is expressly permitted.
- **Removal is silent and retracts completely.** A removed reaction leaves the recipient's view at once and drops out of every notification that named its giver, because notifications render live from current state (§12.2). Where it was the only actor, the notification itself goes: a notification with no actors left is not rendered and is deleted (§12.2). **Nobody is told a reaction was removed** — retraction is never announced (§12.1), and a "someone took back their reaction" notice would be the purest engagement bait this document can imagine.
- **Giving notifies; changing does not.** The first giving of a reaction to an item notifies that item's author under §12.1, coalesced per §12.3. Changing the phrase afterwards generates nothing, for the reason §7.8 gives for not notifying on an edit: a change that costs something socially is a change people stop making, and the whole point of "changeable at any time" is that a reaction can be reconsidered. Removing and later giving again is a new reaction and notifies as one. **Accepted cost, stated:** someone could use that to poke a person repeatedly. It is bounded by the daily reaction rate limit (§13.6) and by unfriending (§5.3) — the platform's answer to a friend behaving badly everywhere else — and it wins nothing measurable, because there is no counter to move.
- **Reaction notifications reach exactly one person:** the author of the item reacted to. A reaction on a comment notifies the commenter and never the post's author, who cannot see it. **Followers of a post (§12.3) are notified of comments only** — a follower cannot see reactions and is never told one happened.
- **A reaction carries no age and no countdown.** §7.5.1's ladder is not applied to it and §7.5's expiry countdown does not appear on it. The reaction line is one line of names (§8.2.2); an age beside each would triple its length and turn a warm line into a log, and a per-reaction countdown would tell the recipient how much longer each piece of warmth has left, which is an archival relationship to one's own reactions and the opposite of what this section is for. Reactions render **newest first**, as every other list on the platform does; with no ages shown, the order is the only thing carrying recency and must be stable across page loads.

### 8.2.2 What a reaction looks like (v1.25)

Before v1.25 this section was written almost entirely in negatives — a builder knew what a reaction must never become and not where it goes on the page. The rules below settle enough to build; every one of them is downstream of a rule already in this document.

**The reaction line (what the recipient sees).**
- It renders on a **single-post view only**, never in a list view — the v1.16 rule above is unchanged.
- It sits **directly beneath the content it belongs to and above that content's comments**: under the post for the post's reactions, under a comment for that comment's reactions.
- It is a **real list in the markup** (§16.3, 1.3.1) with an accessible name — *"Reactions to your post"*, *"Reactions to your comment"* — whose items read *"Alice: Love it!"*. Rendered inline at ordinary widths and stacked when there is no room; any separator between items is presentational. Names render through the shared name helper (§4.5.1) under the link-or-plain-text rule of §8.1 — a reaction from someone the viewer has no connection to renders as plain text, which is possible on a hashtag-gated post (§11.3).
- **It is complete, never truncated, never folded, and never summarized.** "And 4 others" is a number wearing a coat (§17); the countless "and others" of §12.2 is honest there and wrong here, because a notification's job is to say *that* something happened while this line's whole job is to say *who* — an overflow deletes the only thing it carries. Nor is a fold needed: the list is bounded by the audience that could reach the content at all (≤ 30 for a feed post, the author's friends for a profile post) and, since v1.25, by 90-day expiry, so even a pinned post standing for years shows only the last 90 days of reactions.
- **When nothing has been given, nothing is rendered** — no empty state, no "no reactions yet", which is a count of zero written out in words.

**The picker.**
- **Every post and comment on a single-post view carries a react control**, shown to everyone who may see that content, whether or not anyone has reacted. Its presence therefore reveals nothing; only its own viewer's state is in it.
- The control is a **native HTML disclosure** (`<details>` / `<summary>`) — no script, no ARIA menu or tab pattern to implement correctly, the same posture as §9.1's server-rendered tabs. Opening it reveals the phrase list, whose phrases are the buttons of an ordinary form that posts and reloads the page.
- **The control's own text says whether this viewer has already reacted, and with what**: *"React"* when they have not, *"Reacted: Love it!"* when they have. That is how a reactor knows — in **real text**, never a colour or a highlighted button (§16.3, 1.4.1; §16.4). It is also all a reactor ever sees of their own reaction: **the reaction line is not shown to the reactor**, only to the author of the content.
- The list is **`REACTION_SET` in its curated order, one real `<button>` per phrase**, plus a **"Remove reaction"** button when one is set. The currently-set phrase is marked in text, not by colour alone.
- **Distinct accessible names, per §16.3's repeated-controls rule.** A single-post view carries one picker for the post plus one for every comment, so a thread produces many identical controls, and a screen reader's element list of twenty entries reading "React" is the dead end §16.3 already names for "read more". Each control takes a unique name through visually hidden text — *"React to David's post"*, *"React to Alice's comment"* — and so does each phrase button: *"Love it! — react to Alice's comment"*.
- **Touch targets are 44 × 44 CSS px** (§16.3), and **at 320 px the phrase buttons wrap onto as many rows as they need**. Neither the picker nor the reaction line ever scrolls sideways; §7.2.1's preformatted post remains the platform's only reflow exemption (§16.3, 1.4.10).
- **Nothing here is revealed by hovering** (§16.4) — the control is visible at all times, on touch and by keyboard alike.
- Choosing or removing a phrase **reloads the page and renders the new state**; the control's own text is the confirmation. Nothing live-updates and nothing moves under the reader (§16.3, 2.2.2).

### 8.2.3 Curating `REACTION_SET` (v1.25)

The six phrases are placeholders and have been since they were written (Appendix A item 8). **This section states the tests a phrase must pass rather than freezing the list**, which is the same division `HASHTAG_VOCAB` runs on (§11.2.1): the criteria are the durable part and belong here; writing the actual set is content work and belongs to the [FOUNDER+AI] step of the build plan.

**Six criteria.** A phrase is in the set only if it passes all of them.

1. **Warm on the worst post it could land on.** A phrase can be attached to any content its giver may see, including a post about a death or a diagnosis, and the only person who will ever read it is the person it happened to. Any phrase capable of cruelty there does not belong in the set. **This is the criterion that decides the list**, and it is why reactions need no moderation control of their own (below).
2. **A response, not a rating.** *"Great post!"* evaluates the work; *"Love it!"* answers the person. A rating is scorekeeping with a single scorer, which is the thing this section exists to refuse.
3. **No irony carrier.** A phrase whose warm reading depends on context can be aimed. Recorded plainly because it costs the current list a member: **"Ha!" fails criteria 1 and 3** — laughing *with* and laughing *at* are the same three characters and the recipient cannot tell which they were sent — and is **recommended for removal**. **"Thank you!" is recommended in its place**: gratitude is the commonest warm answer to a post about oneself, and the current set has no way to say it.
4. **Plain words, legible without sight.** No emoji, no icons, no punctuation carrying tone beyond a single exclamation mark (§16.3, §16.4). The current set already satisfies this, and it is what makes reactions non-visually equivalent for free rather than by extra work.
5. **Distinct in meaning.** Two phrases that mean the same thing are one phrase and a decision the user should not have to make.
6. **Short enough to be a button** that fits alongside its neighbours at 320 px (§8.2.2).

**The set stays small on purpose.** `REACTION_SET` is ~6 (§14). A set of twenty is a menu, and a menu asks the reactor to hunt for the *right* reaction — which is the evaluative act criterion 2 refuses, reintroduced as an interface. No hard cap is stated, for §11.2.1's reason: it is content work, nothing enforces a number, and the picker's 320 px row is the practical bound.

**Changing the set: retire, never repurpose — and expiry does the migration.**

- **Adding** a phrase takes effect at once and touches no existing reaction.
- **Retiring** a phrase removes it from the picker at once, and **existing reactions carrying it keep rendering it, unchanged, until they expire.** Nothing is rewritten and nothing silently vanishes. Because a reaction lives at most `CONTENT_TTL_DAYS` = 90 days (§8.2.1), **a retired phrase is gone from the platform within 90 days with no migration written and no user-facing event at all.** This is the practical payoff of §8.2.1 and worth naming as one: expiry is what makes the operator's curation cheap.
- **A phrase's text may be corrected, never replaced.** Fixing a typo or a stray character is a correction. Turning "Ha!" into "Thank you!" in the same slot puts words into the mouths of everyone who chose it, which is exactly the ventriloquism §7.8 forbids a host over a comment. **To change what a phrase means, retire it and add a new one** — the "deactivating beats deleting" discipline the URL allowlist already runs on (ARCHITECTURE §4).
- **A reaction whose phrase no longer exists at all renders as nothing and is deleted.** Never a blank, never an identifier, never a placeholder such as *"(removed)"* — a placeholder is a trace, and §7.6 refuses traces of interactions whose whole point was to be impermanent. This should not arise, because retirement is the supported path and deletion is not; it is stated so that a builder has a defined behaviour instead of an empty string.

**No moderation control of its own — considered and rejected (2026-08-18).** A per-recipient *"remove this reaction from my post"* was weighed, as the §8.1 host's-rules power extended to reactions, and **not adopted**. What it defends against is a phrase that lands cruelly, and criterion 1 removes those at curation time — once, for everybody, before anyone is hurt. The control also cannot be made to work cleanly: removal would have to be silent, since retraction is never announced (§12.1), yet a reactor sees their own reaction in the control's own text (§8.2.2) and would learn of it anyway; and a removal the reactor can undo by simply reacting again is not a remedy. The structural answers already exist and are this platform's answers everywhere else — unfriend (§5.3), block (§5.4), report the person or the post (§13.2), and the daily reaction rate limit (§13.6). **If `REACTION_SET` is ever curated loosely enough that this control starts to look necessary, the set is the defect and the set is what to fix.**
...
| **About** | Short bio, extended bio (friends only), interest hashtags, **the people you both know** (§9.2, §11.5), and — friends only — the **contact-card control**: the request, or the link to the card once it has been answered (§10.4, v1.30). |
...
- **The contact-card control is a control, not a field, and the basic-tier invariant below is unaffected (v1.30).** The invariant is stated over the *field set* and its rendering; a friends-only link to another page adds no field to the About tab and appears on none of the three surfaces the invariant governs. It sits here because the card is per-friend (§10.3) and the request has to live somewhere a friend will find it, and because §9.1's header is closed — it carries the photo, the name and the report action, and nothing else.
...
**The basic-tier invariant (amended v1.26).** The header plus the About tab, minus the extended bio, is **exactly** §9.2's basic tier, and **exactly** what §5.2's friend-request card shows. All three render from **one component with one friends-only flag**. This is a requirement, not an observation: if the three surfaces drift apart, the screening arguments of §5.2 and §13.1 quietly stop being true.

**The invariant is over the field set and its rendering, not over the data source.** One component, one template, one list of fields — fed *either* from the owner's current rows *or* from the snapshot §5.2 freezes into a request at the moment it is sent. That distinction is what lets both rules hold at once, and both are load-bearing: a field added to the basic tier appears on all three surfaces automatically, which is the drift this invariant exists to prevent, while a delivered card stays frozen, which is the edit-and-blast attack §5.2's snapshot exists to stop. This is written down because the two sentences read as a contradiction otherwise, and the reconciliation must not be left to whoever builds it: **a builder who reads "one component" as "one data source" rebuilds precisely the attack §5.2 closed.** §9.5's preview-as mode 4 is the same component fed from live rows, and is the standing proof that it takes either source.

**Which fields freeze and which render live.** This list is a security boundary, not a rendering detail, so it is stated rather than derived:

| Field on the friend-request card | Source |
|---|---|
| **Profile photo** | **Frozen** at send time (§5.2) — a stored copy, not a pointer to the current image |
| **Short bio** | **Frozen** at send time (§5.2) |
| Display name | **Live**, through the shared helper — names are never stored on content (§4.5.1) |
| Shared profile hashtags | **Live** — the shared set is computed against the recipient's own current tags, and a vocabulary tag carries no author-written text to abuse (§11.2) |
| The mutual friends they share | **Live** — per recipient, at view time, and subject to §5.4 |
| The report action | **Live** — it targets the requester's profile, not the snapshot (§13.2) |

**The frozen pair is exactly the pair §13.6's send hold covers, and for the same reason:** the profile photo and the short bio are the only author-controlled content the platform pushes at someone who is not the author's friend (§13.1). Nothing else on the card can be rewritten between batches of requests, so nothing else needs freezing — and freezing more would break §4.5.1 for the name and produce a stale mutual-friends line that §5.4 could not empty.

**On the profile itself, every one of these fields is live**; the snapshot exists only inside a delivered friend request, and dies with it at 90 days (§5.2).
...
- **A viewer who may not see a profile gets one response**, identical for a block (§5.4), no mutual friend, a deactivated account (§4.7), a **banned** account (§13.2.1, v1.27) and a profile that never existed. Any variation between those cases is an oracle.
...
- **Disallowed URLs in the short bio are rejected at save (v1.16), and the reason for it was restated in v1.29.** The rule is unchanged: **the short bio cannot deliver a clickable link, and recognizable disallowed URLs are refused** — while obfuscated text ("weebee dash social dot com") remains beyond any such rule, exactly as it is everywhere else. What changed is the justification. v1.16 gave a general one — *an unclickable address is still readable, and a reader can retype it* — and v1.29's link policy makes that reasoning contradict itself: if inert text were never good enough, it would not be good enough on a post either, and §7.2.4 now renders every unapproved address on a post as exactly that. **The rule survives on the right reason instead: the short bio is a push surface.** It is delivered, unasked, to up to 20 people a day in a friend request (§5.2, §13.6), and a surface pushed at people who did not choose it gets the stricter rule. A post is not pushed at anyone in that sense — a reader came to their own feed for it. The outcome does not change; the reason had to, or a careful reader of these two sections finds them arguing opposite things.
- **Extended bio** (About tab, friends only): `BIO_EXTENDED_MAX` = **2,000 characters**, matching `COMMENT_LENGTH_MAX`. Because the cap equals `BLOG_FOLD_CHARS`, the fold can never trigger and the field needs no fold rule of its own. **Friends only** (§9.2), so it is not a push surface: no send hold follows a change to it, and it **follows the uniform link rule of §7.2.4** — allowlisted links clickable, anything else an inert copy box, blocklisted domains refused (v1.29; before that, only allowlisted links were permitted and everything else was rejected at save). Screened at save as the short bio is.
...
§7.5's 90-day rule governs **posts, comments and reactions** — things said at a moment, to somebody, about something. It does not govern **account state**: the profile photo, the gallery, both bio fields, profile hashtags, the contact card, groups, and the friend list. These persist until the user changes or removes them, and are destroyed with the account on erasure (§4.7) or after two years of inactivity (`INACTIVITY_DELETE_DAYS`, §4.8). "Permanent" here therefore means *until you change it, delete it, or go dormant for two years*.

**Reactions were named here in v1.25**, having until then appeared in neither list. They fall on the expiring side, and it is not a close call: a reaction is a statement made at a moment about one specific piece of content — the shortest statement the platform permits, but a statement, attributed by name to the person who made it, and readable by the person it is about. The case that forced the question was the pinned post, where a reaction with no clock of its own would have been permanent; §8.2.1 carries the rule and the reasoning.
...
**Amended in v1.29.** Through v1.28 a card item could only be a phone number, an email address, or a messenger link, which made §7.2.3's own rejection message false: it told users that *"anything else can be shared through the contact methods on the user's contact card"* while §10.2 permitted no such item. The consequence was that WeeBee had **nowhere durable to put "here is my photo album"** — not the card, not the short bio (§9.4, no links at all), not a post or the extended bio (allowlisted domains only). §7.2.4 fixes most of that; this section fixes the rest, and makes the second half of §1.5's principle true.

- Up to `CONTACT_ITEMS_MAX` = 12 items total, drawn from: **phone numbers, email addresses, and links**.
- **A link item may carry any URL, under the uniform rule of §7.2.4** — clickable if the domain is allowlisted **with card scope** (§7.2.3), an inert copy box otherwise, refused if blocklisted. Messenger domains are the card-scoped rows today; a photo-album, file-store or personal-site link is a perfectly ordinary card item and renders as a copy box.
- **Every item carries a label**, up to `CARD_ITEM_LABEL_MAX` (§14) — *"My photos"*, *"Work mobile"* — so that an item is never a naked address. The label is new free text on a surface that had none, and this document is rightly hostile to that, so it gets the short bio's treatment: a **hard length cap**, and **`NAME_BLOCKLIST` screening at every save** (§4.5, §9.4). It carries no links of its own; a URL typed into a label is text in a label, and the label is not a delivery channel.
...
**A card is reportable, and the gate on the `link` kind is lifted (v1.30).** Through v1.29 a contact card was not a reportable object anywhere in this document — report actions existed on posts, comments and profiles (§13.2) and nowhere else. That was tolerable while a card held only a phone number, an email address and a messenger link from an allowlisted domain. It stopped being tolerable the moment a card could carry an author-chosen address **and an author-written label**, delivered to a friend who requested the card: the same shape of gap §13.2 closed in v1.16 when it put a report action on the friend-request card, and for the same reason — *a report the recipient cannot reach is not a defence.*

v1.29 therefore specified the link and label rules and **gated the `link` item kind on the report path being designed properly rather than in the last paragraph of a long session** (founder decision, 2026-09-07). That design is v1.30, and it is **§13.2's "Reporting a contact card"**, which makes a card item a report target in the one operator queue, freezes the card *as that reporter received it*, and says what an upheld report removes. **The gate is lifted: the `link` kind ships with the rest of the card** (BUILD_PLAN Step 9.1), and §10.4 changes with it — the answered card is a page the requester can return to, which is what makes the report action reachable after the moment the card arrives.
...
**The page is resolved live on every visit**, never stored. This is the decision the rest of this section rests on, and it was made on three grounds rather than convenience:

- **A stored answer would defeat §10.3.** The cascade's whole discipline is that accidental over-sharing is unrecoverable and withdrawal must therefore be free and immediate. An item the owner switches off has to *disappear* from the person it was switched off for, which a delivered copy cannot do.
- **A stored answer would be a message**, and it would be the only one on the platform. §10.1 refuses the category, and storing the resolved card for every friend who ever asked — against a report that will almost never come — would put the platform in the business of retaining conversations it exists not to have.
...
**The honest consequence, stated once:** a friend who returns to the page can tell that something changed, because it did. Nothing announces it, exactly as a changed bio announces itself only to the next visitor (§9.4, §12.1) — and the alternative, a card that keeps showing what it showed last week, is the thing §10.3 forbids.

**The card page is where a card is reported** (§13.2). That is the reason this section changed at all: a one-time reply leaves the recipient of an author-chosen address and an author-written label with nowhere to act on it five minutes later, and *a report the recipient cannot reach is not a defence.*
...
**Social activity on your own content.** Comments and reactions on **anything you authored — a post or a comment** — and comments on a post you follow (§12.3). Before v1.15 this document generated neither, so an author learned of a conversation on their own post only by revisiting it — an omission, not a decision. **A reaction notifies exactly one person, the author of the item reacted to** (§8.2.1): a reaction on a comment notifies the commenter and never the post's author, who cannot see it. Giving a reaction notifies; changing one afterwards does not, on §7.8's reasoning about edits. Following a post carries **comments only** — a follower cannot see reactions and is never told one happened.
...
**Everything else.** Friend requests, introduction proposals, invite acceptance, contact-card requests and one-time access-request alerts (§10.5), inactivity warnings (email only, §4.8), **an operator warning (email only, §13.2.1 — v1.27; it is an account notice, and deliberately not an in-feed type)**, and account/security events (new-device login, password change, email change — §4.6.1).
...
- **Rendered live from current state.** Reactions are changeable and removable (§8.2.1) and comments are deletable (§8.1), so a notification renders from what exists at read time, not what existed at event time — the same rule as names in §4.5.1. Removed reactions and deleted comments simply drop out. **A notification left with no actors at all is not rendered and is deleted (v1.25)** — when the last comment on it is deleted or the last reaction removed, the row goes rather than rendering as a nameless event or an empty line. Without that, live rendering would produce the one thing it exists to prevent: a notification that survives the thing it was about.
...
- **Following a post.** Anyone who can currently see a post may follow it and be notified of further **comments** — comments only, since a follower cannot see the post's reactions and is never told one happened (§8.2.1). Commenting turns following on; a visible per-post toggle turns it off, and a global default lives in settings. Recorded deliberately: commenting is treated as a **stated** interest rather than a behavioral inference, which is the line §1.3 draws — it is close to that line and is a decision, not an oversight.
...
- **Email carries no *relative* age (narrowed v1.26).** A relative age is computed when mail is sent and read whenever the recipient opens it — three days later it would be false. The mail client's own received-time is more accurate than anything the body could assert (§7.5.1).
  - **An absolute timestamp has none of that problem, and is not banned here.** *2026-08-04 21:14 UTC* is as true on Friday as it was on Monday. §7.5.1's two exceptions therefore hold in email exactly as they hold on a page: a **security-event mail carries an absolute time** (§4.6.1), and an **inactivity warning states the absolute deletion date** (§4.8).
  - **Why this needed narrowing.** Through v1.25 this bullet banned *every* timestamp while every word of its own justification was about a **relative** age decaying between send and open. The over-reach collided with §4.6.1, which requires the same message to carry an absolute timestamp: *"was that login me?"* is not a question anyone answers with "several hours ago". What is refused in email is the phrase that was true at send and false at read — nothing wider.
...
### 13.1 Layers
1. **Structural** (does most of the work): invite-only vouched entry, no reach, bounded audiences, no strangers, **no free-text messaging channel** — friend requests, introductions and reactions carry no composed text (§5.2, §5.5, §8.2) and there is no DM system at all (§10.1) — and the link policy (§7.2.3, §7.2.4): **no clickable deceptive link can be delivered, no link of any kind is delivered unsolicited, and a blocklisted domain is refused outright** (narrowed in v1.29 to match §7.2.4, which made every other address deliverable as inert text; the claim this layer used to make was that the allowlist blocked *delivery*, and it no longer does).
   **What makes the reaction picker safe is curation, not the absence of text (v1.25).** A fixed phrase list is textless by construction, but "textless" is not the same as "harmless": a warm phrase aimed at the wrong post can still wound, and the recipient is the only person who will ever read it. **§8.2.3's first curation criterion is the control** — a phrase belongs in `REACTION_SET` only if it is warm on the worst post it could land on — and it is why reactions need no moderation surface of their own, a conclusion §8.2.3 reaches explicitly rather than by omission.

**One honest qualification (v1.15, amended v1.16):** this layer is often summarized as "no free-text vectors," and that overstates it. A friend request displays the requester's **own short bio and profile photo** (§5.2, §9.2), both author-chosen, to someone who did not ask for them. That surface is governed not by the absence of text but by four things: screening at every save; **rejection of disallowed URLs in the short bio, not merely de-linking them** (§9.4 — the reason was restated in v1.29: this surface is *pushed* at people who never asked for it, which is what earns it the stricter rule, rather than the general claim that inert text is never enough); a **hold on sending further requests** after either is changed (§13.6); and a **report action on the request card itself** (§5.2), without which the only backstop for an unscreened photo was unreachable by the person it was shown to. The distinction matters because §4.6.1's anti-phishing argument leans on this layer, and an overstated claim is a weak foundation.
**A second qualification, and it is smaller than the first (v1.30).** Since v1.29 a contact card carries an author-written **label** on every item — up to `CARD_ITEM_LABEL_MAX` = 40 characters, up to `CONTACT_ITEMS_MAX` = 12 times (§10.2). That is author-chosen free text reaching another person, and this layer should say so rather than let "no free-text messaging channel" absorb it silently. What makes it a far weaker vector than the friend-request card is that it is **pulled, not pushed**: the reader must already be a friend and must have asked for the card, so there is no unsolicited delivery and no daily budget of strangers to aim at. It is governed by three things — the hard cap, `NAME_BLOCKLIST` screening at every save (§10.2), and, **from v1.30, a report action on the card itself** (§13.2). The third is the one that was missing, and the same sentence applies as applied to the request card: *a report the recipient cannot reach is not a defence.*
...
**Reporting a contact card (v1.30).** A card item is author-chosen content — an address or number the author typed, and a label the author wrote (§10.2) — handed to a friend who asked for the card (§10.4). It becomes a report target here: **a new target type in the one operator queue** (§13.5), on the one report form. No new form, no new queue, no notification to anybody.

**Where the card's action lives.** On the **card page** (§10.4), **one action per item**, as a real `<button>` with visible text — *"Report this item"* — never an unlabelled icon (§16.4). It is available to anyone the page is available to, which is a current friend whose request has been answered, and **not to the card's owner**, exactly as the profile report is not. **An empty card carries no report action**: there is nothing on it to report, and a complaint about the person rather than the card is a profile report.

The per-item placement is the decision here, and the alternative was one action for the whole card. A card may carry `CONTACT_ITEMS_MAX` = 12 items, and an operator opening *"someone reported David's card"* would have to guess which of the twelve. Naming the item costs one button per row, and it is most of what makes the report actionable at all.

**What the report captures.** The card **as that reporter received it**, frozen at report time. Following §9.1's precedent, the frozen/live split is stated as a field list rather than described, because it is a security boundary rather than a rendering detail:

| Field | Source |
|---|---|
| Every delivered item's **kind** — phone / email / link | **Frozen** at report time |
| Every delivered item's **value**, verbatim and unresolved | **Frozen** |
| Every delivered item's **label**, verbatim | **Frozen** |
| The **order** the items were delivered in | **Frozen** — twelve labels can be a sentence, and the order is part of what was said |
| **Which item the report points at** | **Frozen** — a position in the frozen list, never a live pointer to a card item the owner can still edit or delete |
| The **reason category** and the optional note to the operator | **Frozen** — the report's own fields, as for every other target |
| The owner and the reporter | **References**, never names |
| Either party's **display name** | **Live**, through the shared helper — names are never stored on content (§4.5.1) |
| Whether a link item rendered as a hyperlink or as a copy box | **Live** — read from the allowlist at view time (§7.2.4). It is the platform's rendering decision, not the author's content, and freezing it would freeze a judgment the operator is meant to be able to change by editing one row |
| The **cascade that produced this version** — the default, the group overrides, the individual override (§10.3) | **Not captured at all** |
| Status and purge-by date | **Live** — the report's own lifecycle (§13.3) |

Three of those rows are decisions rather than bookkeeping:

- **The whole delivered card is frozen, not only the reported item.** The profile report above freezes the whole render and names a category, and the reason is sharper here: twelve labels of `CARD_ITEM_LABEL_MAX` = 40 characters are up to 480 characters of author-written free text on a surface that had none before v1.29, and what one label cannot carry, twelve can. An operator shown a single label in isolation cannot see that, and would be judging the wrong question.
- **The cascade is not evidence.** What the operator needs is what was *delivered*; how the cascade arrived at it is the owner's private configuration, and groups are private to their owner by rule — members never know they are in one (§6). Capturing the resolution would disclose the owner's group structure to answer a question nobody asked.
- **A frozen card is text and nothing else.** Nothing on a card is an image, so this freeze is a few hundred bytes and carries none of the cost §5.2's frozen photograph carries — which is why §13.3's ordinary lifecycle fits it with nothing added.

**Why the freeze is taken at report time and not at send time.** §5.2's friend-request card freezes at *send*, and that model was tested against this one rather than inherited from it. It does not fit, for reasons specific to how cards are answered:

- **A card is auto-answered on request** (§10.4), so a send-time freeze means storing a copy of every card every friend has ever asked for, kept for as long as the friendship lasts, against a report that will almost never come — and it re-introduces in a place no user can see exactly the stored answer §10.4 refuses.
- **The two freezes defend different things.** §5.2's exists because the friend-request card is *pushed* at up to 20 people a day who never asked for it, and its content must not be rewritten between batches. A contact card is **pulled**, by one friend, who asked. What has to survive here is not the push but the *evidence*, and what endangers evidence is the owner editing the card the moment a report is filed — which report-time freezing answers precisely.
- **The profile report above is therefore the model**, and it is the closer analogue for a second reason: it too freezes a **per-viewer** view — *the profile as it appeared to the reporter* — which is the shape of this problem.

**A target category**, on the profile report's pattern, because the operator needs to know **which part** is wrong; *why* it is wrong is the optional note's job:

> *the label · the address or number · the card as a whole · this person's behaviour.*

**The card as a whole** is reachable from any item's button and is the category for the case where no single item is the complaint. **This person's behaviour** is the same last entry the profile list carries, for a card that is a symptom rather than the thing wrong.

**The freeze lifecycle is §13.3's, unchanged** — dismissed and it is purged at once, upheld and it is purged 30 days after, and 90 days is the hard cap however unreviewed. Both candidate exceptions were checked rather than assumed. It must **not** end early when the owner deletes the item or the friendship lapses: §13.3 already keeps a frozen copy through author-deletion *solely so the report can be judged*, and deleting the reported item is the first thing a reported person does. It needs **no extension** either — a card item never expires (§9.7), so unlike a post there is no case where the frozen copy is the only surviving record of something the platform has since destroyed.

**Accessibility (§16.3, §16.4).** Twelve report buttons on one page are repeated controls, so each carries a **distinct accessible name** naming its own item — *"Report the item labelled 'My photos'"* — taken from the item's label exactly as the gallery's controls take theirs from an image's alternative text (§9.4). An item whose label is empty is named by kind and position instead: *"Report the third item, a phone number."* An element list reading "Report this item" twelve times is the dead end that rule exists to prevent.

**In preview-as (§9.5) the action renders and does nothing.** The preview is a truthful rendering of the surface a friend sees, so hiding the button would misrepresent it, and preview mode is already strictly read-only. No special case is added for it.

**No new rate limit, and that is a decision rather than an omission.** Reports are not among the counted actions of §13.6 and do not become one here. Twelve buttons on a page is not a mass-report weapon, because §13.3 has already removed the prize: reported content **stays live** while a report is open, so filing twelve reports censors nothing and achieves twelve items in a queue the operator reads at their own pace.
...
### 13.2.1 The three outcomes, defined (v1.27)

§13.2's workflow clause names three outcomes — *delete content, warn, or ban an account* — and defines exactly one of them. An external reviewer of v1.16 put the consequence plainly: there is a button in the operator console with no document behind it, and *"different AI models will guess differently — one might make it a permanent deletion, another might make it a reversible deactivation, and a third might only block new posts."* This section is what those buttons do. **What stays deferred is unchanged:** this defines the *outcomes*, never the policy that selects one and never an appeals process (§13.2). A policy is a promise of consistency, and a solo operator who wrote one would then have to keep it.

**Delete content.** Already specified, and unchanged: an upheld report removes the live content and purges the frozen copy 30 days later (§13.3). One clarification, because the ban below turns on the opposite reasoning — deleting a post also deletes its comments (§7.5), so this outcome *does* destroy other people's words. The collateral is bounded to the single item judged abusive, which is what makes it a scalpel; the same collateral applied to a person's whole history would run through every thread they ever joined.

**Delete content, on a contact-card item (v1.30).** §13.2's new target type needed this outcome defined for it, since "delete content" on a card had no meaning anywhere — the same gap this section was written to close for posts, arriving one target type later. **The operator removes the whole item**: the row goes, and it goes from every version of the card at once, for every friend, because there is only one item and the cascade merely decides who sees it (§10.3).

- **Never a partial edit.** Blanking an abusive label while keeping the number, or rewriting either, is the operator writing into somebody's account, and the platform's moderation tools are remove, warn and ban — none of them an edit. Where only the label is abusive the item still goes, and the owner may add the number back under a different label; a card item costs one line to re-create.
- **The collateral here is close to nothing, which is the contrast worth drawing with a post.** Deleting a post destroys its comments — other people's words. Deleting a card item destroys an address the owner typed, and nobody else's anything. It is the cheapest instance of this outcome on the platform, and the freed slot returns to `CONTACT_ITEMS_MAX`.
- **It is silent**, exactly as a deleted post is: reports notify nobody (§13.2), and the item is simply absent from the owner's card editor. If the operator wants the person to know, that is what **warn** is for, and the two compose as they do everywhere else — remove the item, warn the account.
- **If the item is already gone, the outcome is unavailable and the report can still be upheld** — §13.4's content-free counter is the record, and a person who deletes the reported item has not thereby dismissed the report. **If the item still exists but has changed since**, the operator sees both the frozen copy and the current row, and removing it is still the correct act: the reported content existed and was delivered. An operator who judges the change to have fixed it dismisses instead, which is what dismissal is for.

**Warn.** Undefined until now in a second way the reviewer did not name: it had no delivery channel, and §12's notification types did not include one.

- **The channel is email**, sent to the account's one login address (§4.6), on the same system-mail path as §4.8's inactivity notices. It carries **no link** (§4.6.1) and asks for nothing back. No new notification type exists (see the rejection below), so nothing in §12 changes except that §12.1's "everything else" list now names it. **It is not subject to §12's optional-email setting**, which governs social notifications; an account notice is not one, and §4.8's inactivity warnings and §4.6.1's security mails already sit in that position.
- **It never identifies the reporter.** The moderation queue is private (§13.2), and a warning naming who complained makes reporting dangerous — which would cost the platform the only operator-level safety mechanism it has (§13.1 layer 3).
- **It is recorded against the account**: the date, the text the operator sent, and the report it arose from if there was one. *"We warned them twice"* is the ordinary basis for a later ban and nothing stored it before. This is operator-written text about an account, not retained user content, so §13.4's rule is untouched — and §13.4 now says so.
- **An in-feed notification type was considered and rejected (2026-08-18).** A warning is an *account* notice, in the family of §4.8's inactivity warnings and §4.6.1's security events — all system-to-user, all email, none of them in the feed. An in-feed type would make the feed a surface the operator speaks on, would need its own coalescing, read-state and wording rules (§12.2, §12.3), and would reach only a user who happens to log in — the wrong set, since the person most needing a warning may be the one who has stopped visiting. **The honest residual: email delivery is not proof of reading, and v1 requires none.** A warning that must be acknowledged before the account works again is a workflow, and workflows are what §13.2 defers.

**Ban.** A ban is **§4.7's deactivation, done to a person rather than chosen by them, with the session made inert and no clock running.** The mechanism is not new: the hiding a ban needs is already specified, already required for the deletion path, and already the answer §9.3 gives a viewer who may not see a profile. Everything below says only where a ban differs from the four adjacent states — deactivation (§4.7), deletion (§4.7), dormancy (§4.8) and blocking (§5.4).

**1. The account and all of its content become invisible everywhere, reversibly** — exactly §4.7's deactivation, and by the same mechanism: profile, blog, gallery, pinned posts, and the person's comments and reactions on other people's posts. Nobody is notified. Notifications naming them drop out on their own (§12.2 renders from current state), and a visitor to the profile gets the single response of §9.3.

**2. Their words are hidden, never deleted.** A ban is a judgment about a person's future conduct; deleting their past words would also delete half of every conversation they took part in, on the posts of up to `FRIEND_CAP` = 300 people, none of whom were judged. §7.6 states the governing principle in the other direction — *an author may preserve their own words indefinitely, and may never preserve anyone else's* — and this is its mirror: **nor may a judgment about one person destroy the other half of somebody else's conversation.** Hiding achieves everything a ban actually needs (the words are gone from view; no more can be added) at none of that cost, and it is the cheaper build, because §4.7 already specifies it. The operator who needs a specific item *gone* has the delete-content outcome above, and the two compose: delete the post that was reported, ban the account that wrote it.

**3. Signing in still works; the session does almost nothing.** Authentication succeeds and authorization is empty. The person reaches one page, in the platform's ordinary honest register (§13.6), which states plainly that the account has been suspended by the operator and that their content is hidden from other members. **It names no reason** — reasons are the deferred policy question, and a generated reason line would promise a consistency the v1 workflow cannot keep. From that page exactly two things work: **data export (§4.9) and account deletion (§4.7).** Everything else — feed, composers, comment boxes, requests, discovery, the report action, the rest of settings — is that page.
  - **Why not refuse the login outright.** Export and erasure are data rights, not privileges, and a moderation outcome does not cancel them (§4.9, §15.1). Refusing the login moves export to a manual operator job whose first problem is proving that the person emailing is the account holder — the exact thing a password already proves. The session *is* the identity check; making it inert everywhere else is one check in one place, rather than a second and weaker identity process conducted off-platform with someone the operator has just banned.
  - **No session surgery at ban time.** The check runs per request, so an already-open session becomes inert on its next page load.
  - **Self-deletion from that page behaves normally** — §4.7's 30-day grace, then full erasure. A banned person may erase themselves and should be able to: the alternative is a platform holding data hostage as a punishment. Cancelling the deletion during the grace returns them to banned, not to active. Nothing forensic is lost either way: §4.3's anonymized invite-tree stub and §13.4's content-free counters survive erasure by design.
  - **No social email reaches a banned account and none needs suppressing.** An invisible account receives no comments, no reactions and no requests, so nothing generates one.

**4. It is reversible, and reversal is lossy.** Lifting a ban clears the state; everything hidden becomes visible again, and friendships, groups, hashtags and the contact card are as they were. **What does not come back is anything that expired while hidden.** Hidden content keeps expiring on its ordinary 90-day clock — the expiry job does not consult visibility (§7.5) — and so do pending friend requests at 90 days (§5.2) and unredeemed invites at 14 (§4.1). A ban lifted after four months therefore restores an account with no posts in it. **The operator is told this at the moment of banning, not afterwards:** the confirmation states that the ban is reversible, that hidden content goes on expiring, and that nothing which expires can be restored. The record survives the reversal — banned on, lifted on, by whom, with the operator's note (§13.4) — so a second ban reads as a second event rather than as the first one repeated.

**5. Friendships, groups and the contact card are untouched.** They are hidden with everything else and restored intact, because dissolving them would make "reversible" false: an account restored without its friends has not been restored. One consequence is written down here rather than left to be discovered — **a hidden friendship still occupies a slot against `FRIEND_CAP`** (§5.1), so a user can meet the cap while able to see fewer than 300 names. It leaks nothing about who, it is not new (a deactivated friend has always had this property, §4.7), and at this network's scale nobody is near the cap; a builder nonetheless needs the answer, and inventing one would be worse.

**6. Outstanding invitations are voided; nobody's budget is punished.** Unredeemed invitations sent by the banned account are voided on §4.1's ordinary expiry path and return to that account's (inert) budget. An invitation is an act of vouching by the account, and letting one complete after the ban would create a member whose only tie to the network is a hidden account: §4.1 auto-friends inviter and invitee, so the new person would arrive with one invisible friend, no friend-of-friend path to anyone, and no way to be found. **This is a property of invisibility rather than of banning**, and §4.7 now states it once so both states inherit it. In the other direction nothing moves: the invite that created the banned account was spent when it was redeemed, and refunding it to the inviter would attach a consequence to the invite tree in the generous direction, which §4.3 rules out as firmly as the punitive one. The banned person's own invitees are untouched.

**7. The invite tree still attaches no consequence, and this is the moment to confirm it.** §4.3 records ancestry *"for forensics and future analysis (e.g., detecting clusters of abusive accounts)"* and defers any accountability mechanism; §17 keeps it parked. A ban is the event that recording was for, so the answer is confirmed rather than left to be re-derived: **the tree exists so that a human can look, not so that the system can act.** The operator reads it in the console; §13.4's per-account counter, linked to the tree, is already what makes a cluster visible. Reducing an inviter's budget would punish one person for another's conduct, on a judgment a solo operator has not the time to weigh, and would make vouching for a stranger a risk — which is the behaviour §4.1 depends on.

**8. A ban never becomes a deletion on a clock of its own.** Left alone, a banned account's content expires at 90 days (§7.5) and the account itself is deleted by §4.8's 730-day inactivity sweep. **That is the intended end and it needs no machinery** — stated explicitly, because a builder will otherwise wonder whether a banned account should be purged sooner. Two consequences of the working sign-in (point 3) are settled here rather than left to collide with §4.8:
  - **A sign-in to a banned account does not refresh the last-login date for §4.8's purposes.** Otherwise a banned person could postpone the sweep indefinitely by signing in to read the notice, and the one end a ban has would never arrive.
  - **Of §4.8's four inactivity emails, a banned account receives the last two and not the first two.** The 180- and 365-day notes invite a person to come back, which is exactly wrong here. The 670- and 700-day ones carry the absolute deletion date (§4.8, v1.26) and are the last notice that data is about to be destroyed — a data-rights notice, which a moderation outcome does not cancel.

**9. Nothing about a ban is ever visible to another user.** No marker, no tombstone, no explained absence. A comment that was in a thread yesterday is simply not there today, exactly as §7.6 requires for the expired comments on a pinned post, and for the same two reasons: such a marker is a count in disguise, and it is a permanent trace of an interaction. A profile falls to §9.3's single response — identical for a block, a missing mutual friend, a deactivated account, a banned account and an account that never existed — and **§9.3's enumeration now names the banned case**, so the identity of those responses is a stated requirement rather than an inference a builder has to make. The absence goes unexplained on purpose: an explanation would be a public verdict about a person, delivered to as many as 300 people who did not ask for it, and it is not the operator's to publish.

**10. In the console, banning is a per-account action and never a list action.** It lives on the account's own page and is confirmed there, on a page naming the account and stating both what the ban does and what a reversal cannot restore. Django's admin offers a bulk dropdown over selected rows for free, and select-all-then-choose-ban is precisely the mistake one tired operator makes once. Lifting a ban is the same action in the other direction, in the same place. (The console's accessibility commitment is §16.1.1's, and the moderation surfaces are inside the named subset.)

**Blocking authentication outright — considered and rejected (2026-08-18).** It sounds firmer, and it is the same screen with the data rights taken out of it: a correct password already reaches a page saying the account is suspended, whether the platform calls that a login or not. The only real question is whether that page also carries export and deletion, and §4.9 and §15.1 answer it. Rejecting the login would additionally force every future data request from a banned person through an off-platform identity check the operator is worse at than the login form.
...
- **A frozen contact card is an ordinary frozen copy and gets no exception (v1.30).** It is text only — the resolved items as that reporter received them (§13.2) — so it costs almost nothing to hold for the window above, and it must survive the owner deleting the item, which is exactly what the second bullet already promises. This is stated rather than left to inference because a card is the one report target that never expires on its own (§9.7), and a builder could reasonably wonder whether its freeze therefore needs a different clock. It does not.

### 13.4 What persists
Only content-free counters: "account X: N upheld reports," linked to the invite tree for forensics. Never the content itself.

**Two additions in v1.27, both operator-written and neither of them user content.** The **moderation state** of an account — banned or not, with the date of each ban and of each reversal — and the **warnings sent to it**: date, the text the operator sent, and the report it arose from if there was one. §13.2.1 says why the second is stored at all: *"we warned them twice"* is the ordinary basis for a later ban, and nothing recorded it before. The rule this section states is unchanged — **no user content persists here**, and a moderation record is not an exception to that because none of it is anybody's post, comment or reaction.
...
- **External service request** — propose a domain for the URL **allowlist** (§7.2), which is a request to make that service's links **clickable**, and where (posts and comments, cards, or both — §7.2.3's surface scope). Judged against the three admitting categories **and** the anti-phishing bar of §7.2.3, including the open-redirector check. A declined domain is not a domain a user cannot mention: since v1.29 its links are posted as inert copy boxes like any other unapproved address (§7.2.4), so what a decline costs is clickability, not the ability to share. **This channel does not accept blocklist appeals** — §7.2.3 records why, and the refusal message names no such door.
- **Bug report.**
...
| `BIO_EXTENDED_MAX` | 2,000 characters (friends only; follows §7.2.4's uniform link rule) | §9.4 |
...
| `CARD_ITEM_LABEL_MAX` ✎ | **40 characters** (v1.29; the label on a contact-card item — *"My photos"*, *"Work mobile"*. New free text on a surface that had none: capped hard and screened against `NAME_BLOCKLIST` at every save) | §10.2 |
...
| `REACTION_SET` ✎ | ~6 operator-curated phrases. **Retired, never repurposed** — the six criteria a phrase must pass, and what happens to existing reactions when the set changes, are §8.2.3 | §8.2, §8.2.3 |
| `THEME_SET` ✎ | operator-curated fonts + color schemes (each verified against the §16.3 contrast ratios) | §9.1, §16.3 |
| URL allowlist ✎ | operator-curated. Each row carries an **admitting category** and a **surface scope** — posts and comments, contact cards, or both (v1.29); messenger domains are card-only | §7.2, §7.2.3 |
| URL blocklist ✎ | **operator-curated (v1.29)** — domains refused outright, in any form, on every surface. Checked by the same shared validator as the allowlist, on every save path. Retired by deactivating, never deleting; no appeal channel | §7.2.3, §7.2.4 |
| Rate limits ✎ (per account/day) | e.g., 20 posts, 200 comments, 100 reactions, 20 friend requests, 10 intros | §13.6 |
...
Scope: the web application in every state — logged-out pages (login, password reset, invite redemption), the feed, composers, profile pages, discover, settings, contact cards, error and empty states, and every email the platform sends. The **operator console** (Django admin, §13) is a separate and narrower commitment, stated in §16.1.1: it is neither in full scope nor silently exempt.

### 16.1.1 The operator console — a bounded commitment (v1.22)
Through v1.21 this section said the console was "held to the same standard as far as the framework allows; where it falls short, the shortfall is the operator's own, not a user's." The reasoning was real, and on a platform with exactly one operator the distinction between an excluded user and an inconvenienced operator is a genuine one. But the sentence stated **no obligation**: "as far as the framework allows" is satisfied by doing nothing, which is not something §16.1's opening sentence can be read as permitting. **Founder decision, 2026-08-18: the console is in scope for a named subset and out of scope beyond it.**

**In scope — and verified, not assumed.** Over the operator surfaces of §13 that are used in an ordinary week — the moderation queue, the request/suggestion queue (§13.5), the hashtag vocabulary editor (§11.2, §11.2.1), and the URL allowlist and blocklist editors (§7.2.3):
- **Keyboard operability with no traps** (2.1.1, 2.1.2). Every action an operator performs weekly is reachable and completable without a mouse.
- **A visible focus indicator** (2.4.7).
- **Text contrast ≥ 4.5:1, UI boundaries ≥ 3:1** (1.4.3, 1.4.11), in whichever admin theme ships.
- **Every form field programmatically labelled** (1.3.1, 3.3.2), and **errors identified in text and associated with their field** (3.3.1).

These are checked once, at build-plan Step 16.5, in the same sitting and by the same tester as the user-facing audit (§16.5.1). The subset is chosen for three reasons: it is close to what Django's admin already delivers, it is cheap to verify, and it is the part whose absence makes a surface *unusable* rather than merely graceless.

**Out of scope, with the reason stated.** Everything else — screen-reader announcement quality, reflow at 320 px (the changelist is a data table, which 1.4.10 exempts on its own terms), live-region status messages, and every criterion that would require editing Django's own admin templates. **The reason is not that operators matter less.** It is that conformance here would mean *owning the templates*, and owning them means either forking an upstream this project must keep patched for security, or replacing the admin with a hand-built operator interface — a reversal of ARCHITECTURE §3.1 leg 2, where "the Django admin is the operator console for free" is one of the five reasons this stack was chosen at all. A prototype for a few dozen friends does not spend weeks of its only builder's time there. This is a cost/benefit call in the manner of §13.2's declined similarity detection, not a principled objection.

**The consequence, acknowledged rather than assumed away.** "The operator is me, and I can see fine" is a statement with an expiry date: §15.3 contemplates funding phases and §2 contemplates going public, and both are futures in which somebody else holds the role. **If the operator role is ever held by a person the Django admin excludes, that is a problem this project has committed to solving then** — by building the operator surfaces it needs as first-party pages, which is feasible precisely because they are a small number of list-and-act screens, and **never by an overlay** (§16.4 admits no exception, and the ban is not narrower for the operator's own screens). A deferral with a named trigger and a named remedy is a decision; "as far as the framework allows" was not.

**The claim says which.** The accessibility statement (§16.5) states the scope of what is claimed — every user-facing surface — and names this carve-out in a sentence. "We conform to WCAG AA" with a silent exclusion inside it is exactly the claim the honesty rule exists to prevent.
...
- **Repeated controls carry distinct accessible names (v1.16).** A list of posts produces many identical "read more" controls, and a screen reader's element list of twenty entries reading "Read more" is a dead end. Each is a real `<button>` (it navigates nowhere) with `aria-expanded`, a unique name — *"Read more of David's post from a few days ago"* — via visually hidden text, and focus that stays on the control as it becomes "Show less". The same rule governs the gallery's per-image controls, which take their names from the image's own alternative text (§9.4); the **reaction picker** of §8.2.2, which produces one control for the post plus one for every comment on a single-post view; the **copy control on a link's copy box** (§7.2.4, v1.29), of which a single feed page may carry many — its accessible name has to say which address it copies, and the copy is confirmed in a polite live region; and the **per-item report action on a contact card** (§13.2, v1.30), of which one page may carry `CONTACT_ITEMS_MAX` = 12, named from the item's own label and, where there is none, from its kind and position.
...
- Reactions (§8.2) are curated **phrases**, not emoji or colored icons, and are therefore already non-visually legible — a happy consequence of the no-scorekeeping rule. Whether the viewer has already reacted, and with which phrase, is likewise carried in the react control's own text and never by colour (§8.2.2).
...
### 16.5 Verification, re-verification, and feedback
**Conformance is verified, not assumed — and verified on three clocks, not one (v1.22).** Through v1.21 this section described a single pre-launch audit. A gate is a poor instrument for a property that new features keep breaking: §17's "everything after this is SPEC v1.16+" means the platform keeps growing, and **accessibility regressions are the easiest defects to introduce and the hardest to notice.** Nothing looks wrong, no test fails, nobody files a bug — the person who can no longer use the page simply stops using it, on a network they were invited to by a family member. So conformance is held on three cadences:

1. **Continuous — every change, forever.** Two mechanical checks are ordinary automated tests that run beside every other test, at every build step and every change after launch: the **contrast test** over every `THEME_SET` combination, including the viewer-override combinations of §9.1; and the **template smoke tests** over rendered HTML — every `<img>` carries an `alt` (the explicitly empty one where the uploader marked it decorative), every input has an associated label, every page has exactly one `<h1>` and a `<title>`, no positive `tabindex`, and no `<div>` carrying a click handler where a `<button>` belongs. **These are not part of the audit; they are what should make most of the audit a formality by the time it runs.** A change that breaks one fails the build on the day it is written, naming the theme or the template.
2. **Per milestone — the automated scan.** An axe/`pa11y` scan over a locally served instance, at each build-plan phase milestone rather than once before launch. It stays **local tooling and never an application dependency** (ARCHITECTURE §9) — nothing accessibility-related ships to any browser, which is what §15.2 requires and what this satisfies. It finds perhaps a third of real problems, which is a reason to run it a dozen times rather than once.
3. **Periodic — the human passes.** Keyboard-only, screen reader, and 200 %-zoom / 320 px-reflow, over every page type. No automated tool catches "the focus order is bewildering," and roughly a third of WCAG failures are findable no other way. **These are run by a qualified tester rather than the founder (§16.5.1)**, at launch and whenever the rule below fires.

**The re-audit rule (v1.22).** Deliberately modest, because each run now costs another person's time and a calendar: a rule the operator skips is worse than a rule that is small. The human passes repeat when any of these is true.
- **A new interactive pattern ships** — a control type not already audited: a new picker, a dialog, an expandable, a reordering control, a new composer mode. The re-audit is **scoped to that pattern and the flows it appears in**, not a repeat of the whole sweep. **A new page assembled entirely from existing shared partials is not a trigger**; that is what the single-source partials of ARCHITECTURE §3.8 exist for, and this is the practical reward for having used them.
- **Before any widening of who can join** — a raise of the invite constants (§14) or any public phase (§2, §15.1). All three passes, over everything. This is the same moment §15.1 requires attorney review, so the two are scheduled together and neither arrives as a surprise.
- **A user reports an accessibility problem** (below) that the continuous checks did not catch. The report is fixed on its own terms, and **the pattern it belongs to is re-audited** — a report is evidence that a pattern got through, not merely a page.

**Plus one founder-run check a year**, in the sitting that already exists for the yearly restore rehearsal (ARCHITECTURE §10): keyboard-only and 200 %/320 px over the main flows. It is not a substitute for §16.5.1's tester and is not claimed as one. Its purpose is different: the site can stand perfectly still while browsers, operating systems and screen readers move underneath it.

- An **accessibility statement page** is published alongside the privacy policy and terms (§15.1): the conformance target, **the scope of the claim** including §16.1.1's operator-console carve-out, the date and method of the most recent verification, known limitations honestly listed (including the preformatted-post exemption of §16.3), and how to report a problem. **It is a living document, not a launch artifact (v1.22):** it is revised whenever the human passes repeat, whenever a listed defect is fixed, and whenever a new one is found and not yet fixed. A statement that is written once and never revised becomes false by standing still — so letting it go stale is a breach of the honesty rule below, not untidiness.
- The operator request form (§13.5) gains an **"Accessibility problem"** category. Accessibility reports are triaged ahead of feature requests. **They are also the one queue item not read in a batch (v1.22):** everything else there can wait for the weekly sitting, but this one is the signal that both the continuous checks and the audit missed something, and it arrives from the person being excluded.
- **Honesty rule:** the platform claims exactly the conformance it has verified. A known unfixed defect is listed in the statement rather than papered over — **including a defect the tester finds and the founder cannot fix.**

### 16.5.1 Who runs the human passes (v1.22)
**Founder decision, 2026-08-18: the three human passes are run by a qualified tester, not by the founder.** The automated checks stay the founder's.

The reason is that those passes are a *skill*, and the documents had been treating them as a *setting*. Build-plan §2.5 teaches how to turn VoiceOver on; using one well enough to find defects means navigating by headings, landmarks and form controls, and telling a missing label from a live region that never fired. A competent IT professional doing it for the first time will miss real failures and chase false ones for hours — and until this decision, that pass was the only quality gate standing between the platform and its first blind user.

**Qualified means one of two things, and the first is preferred: a person who uses a screen reader daily as their primary means of using the web**, offered payment for their time; failing that, a **paid professional accessibility auditor**. The preference is deliberate. An auditor reports that an accessible name is missing; a daily user reports that the page cannot be used — which is the question this section is actually asking. And a daily screen-reader user is precisely the person §16.2 says an invite-only network of friends and family must not exclude; engaging one to test the door is that commitment expressed as an action rather than a paragraph.

Two consequences are carried in the build plan rather than left as intentions: **the tester is lined up as a step of its own, well before the gate** (BUILD_PLAN §15.3), because a person must be found and booked and Phase 16 had assumed the founder was available to himself; and **the same sitting covers §16.1.1's operator-console subset**, which is a short addition to a visit already being paid for.

**Re-audits run on the live system, and that has a privacy consequence (v1.22).** The launch audit is harmless — it happens before BUILD_PLAN §17.1 wipes the seed data, so nothing the tester sees belongs to anybody. Every later re-audit does not have that luxury: there is one server (ARCHITECTURE §2, Decision 2) and no staging copy of it. **So a returning tester works from an ordinary member account whose only friend is the founder.** They then see the founder's own posts and profile and nothing belonging to anyone else, which is exactly what §5 and §9 already guarantee any member — no exception is made, no operator view is granted, and the visibility engine is not asked to make one. Where a pattern cannot be exercised alone, **the second participant is the founder**. This costs a little realism in the test and is the right trade: an audit that requires seeing strangers' content to be thorough would be verifying accessibility by breaking the thing accessibility is here to protect.

**One accepted cost, named rather than solved.** This makes a re-audit depend on one particular person's continued willingness, in the same shape as the vocabulary's dependence on one operator (§11.2.1) and with the same honest mitigation: **an unavailable tester delays a re-audit; it does not break anything that already works.** The continuous checks keep running, the shipped patterns keep behaving as they were verified to behave, and the cost of a gap is a new pattern going unaudited longer than intended. If the person becomes unreachable, the fallback is the professional auditor named above, and the rule is that the trigger stays open until it is run — a re-audit that was due and did not happen is a known limitation, and the honesty rule puts it in the statement.

---

## 17. Non-Goals (explicit)

No reshare/repost/quote of any kind · no public content or logged-out visibility · no likes or visible counts (followers, reactions, views, post counts, page numbers — none exist) · no algorithmic feed · no DMs · **no global search of people or content** — filtering one's *own* friend list is navigation, not search, and is provided (§11.6); searching the interest vocabulary (§11.2.1) is search over a static operator-written list that names no person and no post, and is likewise not this · no global hashtag browsing (the only tag view is the viewer-scoped discover filter, §11.4) · **no infinite scroll and no "load more"; long lists page with prev/next links (§7.7.1)** · **no date-based archive navigation** (absolute dates appear nowhere in the interface, §7.5.1) · **no gallery captions** — the alternative text is the caption (§9.4) · **no expiry of profile photo, gallery, bios, hashtags, contact card, groups or friend list**: statements expire, descriptions do not (§9.7) · no events system (delegated to external services, §1.5, §7.2.3) · no video/audio · no native app (v1) · no API for third parties (v1) · no AT Protocol / blockchain · no minors · no businesses · no CAPTCHA or third-party human-challenge (§4.6.1, §16.4) · no automated content-similarity or duplicate-post detection (§13.2) · **no structured profile fields** — no relationship status, location, birthday, employer or equivalent (§9.6) · no unread-count badges and no follower counts (§12.2, §12.3) · no absolute timestamps in the interface (§7.5.1) · no end-to-end encryption or per-post content signing in v1 (§15.5; encryption-at-rest is used instead) · **no accessibility overlay widgets and no separate "accessible version" of the site (§16.4)** · no WCAG Level AAA conformance claim (individual AAA criteria are adopted where free — §16.1) · **no full WCAG AA claim for the operator console** — the Django admin is committed to the named subset of §16.1.1 and no further, with the reason and the future-operator remedy stated there · no machine-generated image descriptions (§16.3) · defending against off-platform screenshots is out of scope · **defending against manual re-propagation is out of scope** — a human retyping content, pasting a link, or running a "copy this into your feed" chain letter is not designed against, because it is indistinguishable from a friend telling a friend; the no-reach guarantee covers mechanical propagation, and §1.2 states plainly why the manual path stays self-limiting.
...
**Which document owns a rule, and what the others are doing when they repeat it (v1.24).** These documents restate each other constantly, and on purpose: a rule stated once in a thousand-line document, read by a fresh session that was handed a *different* document, gets missed — and the argument that travels with a rule is what stops a builder treating it as optional. **The redundancy is a reliability measure and is not being removed.** What it needs is not deletion but an owner.

- **This document owns behaviour** — what a user can do, what they see, what the platform guarantees and what it refuses. Where it names a mechanism (one shared helper, §4.5.1 and §7.5.1; Argon2id, §4.6.1; revalidation on every save, §7.8), it is because the guarantee is not checkable without naming it, and the mechanism is a requirement rather than a suggestion passed downstream.
- **ARCHITECTURE.md owns implementation** — technology, data shape, module boundaries, where a check physically lives, and what it costs. It also owns rules this document does not state at all: the visibility engine, the request-scoped memo, the caching ban, the job discipline. Where it restates a rule from here, it says so.
- **BUILD_PLAN.md owns sequence and verification** and defines no rule of its own. Its Appendix is a restatement of both documents above, cited rule by rule.
- **A restatement carries a citation naming its owner.** Where two documents say the same thing, the citation is what says which copy to change first. Nothing is deleted to remove a duplicate, and no reasoning is stripped to make a copy shorter — a bare instruction is the easier one to skip, which is the failure this arrangement exists to prevent.
- **Notation.** `SPEC §x`, `ARCHITECTURE §x` and `BUILD_PLAN Step x` name another document. A bare **§x** always means a section of the document you are reading — except inside a list a document name already governs (`SPEC §7.7, §12`), where the name carries across the list. This is load-bearing rather than tidy: SPEC and ARCHITECTURE section numbers collide across most of their range, so an unlabelled §7.2 is *Content rules* in one document and *What keeps it running* in the other.

**A cleaner split was proposed and declined (2026-08-18).** The suggestion was to divide every such rule in two — this document keeping *"editing must perform the same validation as creation"* and ARCHITECTURE taking *"implemented by routing both through the shared validator."* Tested against the four passages it was offered for, it did not survive: §7.8 invariant 4 and §4.6.1 already sit on the right side of that line and carry only the argument twice, which is deliberate; §7.9's *"one string per post, not a per-viewer computation"* looks like a performance instruction and is in fact the privacy rule of the bullet it belongs to, restated; and the single-shared-helper rule cannot leave this document without contradicting the Purpose line above, which promises a builder can work from it. The division was declined and the citation convention adopted instead; the full reasoning is in CHANGELOG 1.24.
