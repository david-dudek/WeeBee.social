...
**This file last changed in:** 1.25 (§4 gives `reactions` its own expiry clock and adds the `reaction_phrases` table; §4 and §6 delete a notification left with no actors; §6's `expire_content` sweeps reactions by their own timestamp)
...
- **comments** — flat, on posts; own 90-day clock; nullable **`edited_at`** under the same display-only rule. Editing is author-only: the post author's delete power (SPEC §8.1) must not be wired to an update path.
...
- **reactions** — (user, target post or comment, a reference to a `reaction_phrases` row, and **`set_at`**); one per user per target. Three properties the model must carry rather than leave to a view:
  ...
  - **Visible to exactly one person**, the author of the target row (SPEC §8.2). On a comment that is the commenter, **never the post's author**. This is a visibility question and belongs to the engine (§5), not to a template that happens to know whose page it is on.
