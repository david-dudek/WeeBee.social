...
**This file last changed in:** 1.29 (§4 gains the `url_blocklist` table and surface scope on `url_allowlist`; §7's link validator becomes a three-outcome classifier)
...
- **contact_items** — up to `CONTACT_ITEMS_MAX` = 12 per user: kind (**phone / email / link**), value, **label**, default on/off. **The `link` kind replaces `messenger-link` (SPEC §10.2, v1.29)** and holds any URL: what the kind no longer decides is whether the link is *clickable*, which is now a property of the destination and the surface, read from `url_allowlist` at render time (§7 classifier, card scope). Storing "is this clickable" on the item would freeze a decision the operator is supposed to be able to change by editing one allowlist row. The **`label`** column is new free text — ≤ `CARD_ITEM_LABEL_MAX`, **screened against `name_blocklist` at every save**, exactly as `short_bio` is (§7, SPEC §10.2) — and it is never linkified. **A card item is not reportable yet and link items do not ship until it is** (SPEC §10.2): the report path is its own design session, and the constraint it has to satisfy is that a card is per-requester (SPEC §10.3), so the operator needs the version the reporter received, not the owner's current rows.
