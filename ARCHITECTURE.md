...
**Citations:** `SPEC §x` and `BUILD_PLAN Step x` name another document; a bare **§x** is a section of *this* one. Where this document restates a rule SPEC owns, the citation says so — SPEC §18 carries the convention and the reason it matters.
...
4. **Fewer moving parts, so fewer places to be wrong.** Less coordination between separately-deployed parts means less room to wire something wrong — a property that holds whoever writes the code, and one a solo builder reviewing someone else's work needs most. It pays double here, and the second reason is named rather than left implied: SPEC §2 requires these documents to be executable by **less capable AI models**, and coordination across a client/server split is exactly what those get wrong.
...
So the architecture mandates **one module of code that answers all visibility questions**, and everything else must ask it. Roughly six questions cover the platform:

- `can_see_post(viewer, post)` — implements SPEC §7.4 + §11.3
- `can_see_comment(viewer, comment)` — the post's audience **minus anyone blocked with the comment's author** (SPEC §8.1, §5.4). **New in 1.26, and it is not a synonym for the line above:** SPEC asserted the two audiences were identical until v1.26, which is exactly why no such function existed and why every template rendering a comment was left either applying the block rule itself or not applying it at all
- `can_see_profile_tier(viewer, owner)` — returns nothing / basic / full (SPEC §9.2)
- `visible_contact_card(viewer, owner)` — the cascade with deny-beats-allow (SPEC §10.3)
- `are_connected(a, b)` — friend / FoF / stranger, with blocks short-circuiting everything (SPEC §5.4)
...
- **Blocks become reliable:** because *every* list, count, suggestion, and page flows through `are_connected`, the "fully mutually invisible" rule (SPEC §5.4) is enforced in one place instead of forty.
...
2. **The Django admin is the operator console for free.** Django auto-generates a private, login-protected administration interface over the database. That is, nearly verbatim, SPEC §13's operator needs: the moderation queue, the request/suggestion queue (SPEC §13.5), the hashtag vocabulary editor (SPEC §11.2), the URL allowlist editor (SPEC §7.2), ban/warn actions. Building an operator backend by hand would otherwise be weeks of work; here it's configuration.
3. **Strong conventions and secure defaults — the safest stack for whoever assembles it.** Django has one obvious place for everything, and its defaults are secure rather than merely available, so the cheapest way to write something is usually also the correct way. A stack that requires assembling ten libraries by hand (the Node.js failure mode) has neither property, and that is a hazard for any inexpert builder, human or otherwise. **The AI-capability argument is the same point sharpened, and it is stated second on purpose:** Django is among the best-represented frameworks in every coding model's training data, so a **less capable model** makes fewer and less dangerous mistakes in it than in an assemble-it-yourself stack. That reason is real and this project's method depends on it (SPEC §2) — but the leg stands without it, which is why it is not the leg's name.
...
- **Vanilla JavaScript, hand-written, self-hosted** for the handful of dynamic pieces: the audience picker with its live ≤30 count (SPEC §7.3), the searchable hashtag picker (SPEC §11.2), the contact-card visibility toggles (SPEC §10.3), image upload preview, "save this selection as a group?" (SPEC §6). Each is an island of a few dozen lines on an otherwise static page. If these grow tiresome, **htmx** (one small self-hosted file that lets HTML fetch HTML) is the approved escalation — never React/Vue (Decision 1).

### 3.6 Email: the one outside service, and why it's unavoidable

The platform must send email: verification links, password resets, invites, optional notifications, and the legally-serious inactivity warnings (SPEC §4.8) and deletion notices. Sending email directly from a VPS **does not work in practice** — the big mailbox providers distrust small servers, and messages silently vanish into spam. Everyone, including one-person projects, uses a **transactional email provider**: a service with an API and a reputation that gets mail delivered.
...
- **The operator console is a bounded commitment, and the boundary is architectural** (SPEC §16.1.1, decided 2026-08-18). The Django admin is held to keyboard operability with no traps, visible focus, text contrast, labelled fields and text errors — verified once, over the four surfaces an operator actually uses weekly. Beyond that it is out of scope, and the reason lives here rather than in SPEC alone: conformance would mean **owning the admin's templates**, which means either forking an upstream this project must keep patched for security, or replacing the admin with a hand-built operator interface — a reversal of §3.1 leg 2, where "the Django admin is the operator console for free" is one of the five reasons this stack was chosen. **Two things follow for a builder.** The admin's templates are not forked or patched to chase criteria. And if the operator role is ever held by someone the admin excludes, the remedy is **first-party operator pages**, built from the same partials as every other page — never an overlay, which SPEC §16.4 bans with no exception for the operator's own screens.
...
- **friend_requests** — pending/declined with timestamps (drives the 90-day cooldown, SPEC §5.2).
- **blocks** — one row per blocker→blocked; the visibility engine checks it before everything.
- **groups** / **group_members** — private per owner; ≤30 members (SPEC §6).

**Invitations**
- **invites** — code, sender, expiry, redeemed-by.
- **invite_tree** — permanent parent→child records; on account deletion the child's node is replaced by an anonymized stub (SPEC §4.3, §4.7).
...
- **post_audience** — the posting-time snapshot: one row per (feed post, recipient). SPEC §7.4's rule = "row exists here AND friendship currently exists." **An edit never writes to this table** (SPEC §7.8 invariant 1); the one thing an edit *can* change is a profile post's rows in `post_hashtags`, because SPEC §11.3's gate is evaluated live by design.
- **comments** — flat, on posts; own 90-day clock; nullable **`edited_at`** under the same display-only rule. Editing is author-only: the post author's delete power (SPEC §8.1) must not be wired to an update path. **A comment's audience is not its post's (v1.26):** a block removes the blocked party from the comment's audience while leaving them in the post's (SPEC §8.1, §5.4), so a comment list is an engine-owned queryset and never `post.comments.all()` in a template (§5.1, §5.4). The distinction is invisible in ordinary data and shows up only where a block exists.
...
- **reactions** — (user, target post or comment, a reference to a `reaction_phrases` row, and **`set_at`**); one per user per target. Three properties the model must carry rather than leave to a view:
  - **`set_at` is an expiry clock of its own** (SPEC §8.2.1). A reaction dies 90 days after it was last set, **whether or not its parent post is pinned** — so `expire_content` (§6) sweeps reactions by this timestamp and not only as a cascade from a deleted post. A pinned post is never deleted, which is exactly the case that made the sweep necessary.
  - **Changing a reaction rewrites `set_at`.** This is the one deliberate difference from `posts.edited_at` and `comments.edited_at`, which never touch a clock (SPEC §7.8 invariant 2); SPEC §8.2.1 records why the two cases differ. Nothing else about a reaction is versioned — there is no edit marker and no history.
  - **Visible to exactly one person**, the author of the target row (SPEC §8.2) — or to **nobody**, where a block now stands between that person and the reactor (SPEC §5.4, v1.26). On a comment the one person is the commenter, **never the post's author**. This is a visibility question and belongs to the engine (§5), not to a template that happens to know whose page it is on.
- **reaction_phrases** — the operator-curated `REACTION_SET` (SPEC §8.2, §8.2.3), edited in Django admin alongside `hashtag_vocab` and `url_allowlist` (BUILD_PLAN Step 13.3). **A table, not a constant in `constants.py`**, because the operator edits it at runtime; `REACTION_SET` names the table's contents, not a Python literal. Each row carries the phrase text, a display order, and an **active** flag. **Rows are retired by clearing `active` — never deleted, never repurposed:** existing reactions render live from the row, so deleting one would blank somebody's statement and rewriting one would change what they said (SPEC §8.2.3 — the same "deactivating beats deleting" discipline as `url_allowlist` below). A retired row leaves the picker at once and stops being referenced within 90 days on its own, because reactions expire, so **no migration is ever written**. A reaction whose phrase row is genuinely gone renders as nothing and is deleted — never a blank, an id, or a "(removed)" placeholder.
- **hashtag_vocab** — the operator-curated vocabulary (SPEC §11.2), edited in Django admin. Each row also carries **search aliases** (SPEC §11.2.1) — the synonyms a user might type for that interest. They are matched by the picker's search and **never rendered, never selectable, and never joined to `profile_hashtags` or `post_hashtags`**: an alias is a route to a tag, not a tag. Storing them on the row rather than in a table of their own is deliberate — they have no identity apart from their tag, and nothing ever queries them except that one search.
...
- **profiles** — the two bio fields of SPEC §9.4, which are **not interchangeable and must not be collapsed into one column with a length check**: `short_bio` (≤ `BIO_SHORT_MAX`, basic-tier/FoF-visible, **rendered with links disabled**) and `extended_bio` (≤ `BIO_EXTENDED_MAX`, friends-only, following SPEC §7.2.4's uniform link rule — allowlisted links clickable, anything else a copy box, blocklisted refused). Also gallery membership, **profile-page theme choice** (what visitors see, SPEC §9.1), the owner's **own-app-view theme**, and the **"always use my own theme" viewer override flag** (SPEC §9.1 v1.4). Two timestamps drive the SPEC §13.6 cooldown: **`short_bio_changed_at`** and **`photo_changed_at`**, each starting a `BIO_CHANGE_COOLDOWN_HOURS` clock with a `BIO_EDIT_GRACE_MINUTES` free window measured from the *first* save in a burst — so the stored timestamp is the burst's start and is **not** rewritten by a grace-window save. Clearing `short_bio` to empty bypasses the check entirely, and a save rejected by screening writes nothing, so it cannot start a clock.

**Contact cards (SPEC §10)**
- **contact_items** — up to `CONTACT_ITEMS_MAX` = 12 per user: kind (**phone / email / link**), value, **label**, default on/off. **The `link` kind replaces `messenger-link` (SPEC §10.2, v1.29)** and holds any URL: what the kind no longer decides is whether the link is *clickable*, which is now a property of the destination and the surface, read from `url_allowlist` at render time (§7 classifier, card scope). Storing "is this clickable" on the item would freeze a decision the operator is supposed to be able to change by editing one allowlist row. The **`label`** column is new free text — ≤ `CARD_ITEM_LABEL_MAX`, **screened against `name_blocklist` at every save**, exactly as `short_bio` is (§7, SPEC §10.2) — and it is never linkified. **A card item is a report target from 1.30 and the gate on link items is lifted** (SPEC §10.2, §13.2). The constraint it had to satisfy was that a card is per-requester (SPEC §10.3), so the operator needs the version the reporter received rather than the owner's current rows; the answer is a **report-time freeze taken from the engine** (§5.1) and stored on `reports`, not a second copy of anything here. Nothing on this table changes for it: an item is still one row, still edited and deleted freely by its owner, and a frozen report copy deliberately holds no pointer back to it.
...
- **card_requests** — who asked whom and when. **It records *that* the request was answered, never the answer (corrected in 1.30).** The earlier wording — "what was auto-answered" — read as storing the resolved card, which SPEC §10.4 now refuses in as many words: the card page is re-resolved live on every visit, so a stored answer would keep showing an item the owner had switched off, and would be the only stored message on a platform built not to have any. The row exists to record that the request happened (it is rate-limited and it notifies the owner once, SPEC §12.1, §13.6) and to open the card page for that pair; **a return visit writes nothing here and notifies nobody.**
- **access_flags** — the request-more-access flags (SPEC §10.5; may ship v1.1 — table costs nothing to design now).

**Flows and operations**
- **introductions** — broker, the two candidates, per-side accept/decline state (SPEC §5.5).
- **notifications** — the in-feed items (SPEC §7.7, §12). Each row carries: recipient, **event kind** (blog post / photo change / gallery addition / comment activity / reaction activity / friend request / introduction / card request / security event …), a reference to the subject, read state, and a **coalescing key**. Three properties matter more than the columns:
  ...
  - **Actors are rendered live, never stored as text.** A notification stores actor *references*; the display name comes from the shared name helper (SPEC §4.5.1) and reactions/comments that were removed simply drop out at render time (SPEC §12.2). Storing "Alice and Tom commented" as a string would freeze a deleted comment into the feed and re-break the rule that names are never snapshotted onto content. **A row left with no actors is not rendered and is deleted** (SPEC §12.2, v1.25) — the last comment deleted or the last reaction removed takes the notification with it, rather than leaving a nameless event in the feed; `expire_notifications` (§6) sweeps any the render path did not catch.
...
- **reports** — reporter, target, category, **frozen copy**, status, purge-by date (SPEC §13.2–13.3); handled in Django admin. The `category` column carries the **reason** every report now has (SPEC §13.2, v1.21) — the profile target categories, and for posts and comments a short reason list including *the tags don't match this post*. It already existed; v1.21 only fixed which reports populate it. There is deliberately **no index or aggregate that counts reports per hashtag** (SPEC §7.9).

  **The contact-card target type (new in 1.30, SPEC §13.2).** A fourth target joins post, comment and profile, and it is the first whose frozen copy is **not** a copy of a stored row. What is frozen is the **resolved card as one reporter received it** — an ordered list of `{kind, value, label}` plus the **position** of the item the report points at — captured by calling the engine's `visible_contact_card(reporter, owner)` **once, at submit** (§5.1). Four properties are load-bearing and each is a way this gets built wrong:
  - **It is a value, not a reference.** The frozen list holds no foreign key to `contact_items`; the reported item is identified by its position in the frozen list. A pointer would be defeated by the owner deleting the row, which is the first thing a reported person does.
  - **It stores no resolution trace** — no group ids, no override rows, nothing about *how* the cascade produced this version. Groups are private to their owner (SPEC §6), and the operator's question is what was delivered.
  - **It is text.** No image, no file, no path — nothing on a card is an image — so `purge_moderation` (§6) needs no new file-deletion branch for it and the row is a few hundred bytes.
  - **Clickability is not part of it.** Whether a `link` item rendered as a hyperlink or a copy box is re-derived from `url_allowlist` at view time by the same classifier as everywhere else (§7), for the same reason it is not stored on `contact_items`.

  The **upheld action** for this type deletes the `contact_items` row the report names, if it still exists (SPEC §13.2.1) — never a partial edit of one, and never anything on `contact_overrides`, which is the owner's configuration rather than content.
- **operator_requests** — the SPEC §13.5 form submissions (category, text, submitter).
- **upheld_report_counters** — the content-free per-account counts that outlive everything else (SPEC §13.4).
- **job_runs** (v1.20) — one row per run of every §6 command: job name, started/finished timestamps, outcome, a short detail string, and a count of items acted on. **Content-free by design** — it counts, it never names — so it says nothing SPEC §15.2 would object to being kept. Written in a `finally` block, so a job that crashes still records a failure rather than leaving no row at all, which is the case that matters: this table is the only thing that distinguishes "the job ran and found nothing to do" from "the job has not run since Tuesday" (§7.3). Read by `check_health` and visible in Django admin.
- **rate_counters** — per user/action/day counts for SPEC §13.6 (in the database, not Redis — Decision 5's simplicity again). Note that **three of SPEC §13.6's controls are not day counters and do not live here**: the feed-post spacing (`POST_MIN_INTERVAL_MINUTES`) and the two bio/photo cooldowns (`BIO_CHANGE_COOLDOWN_HOURS`) are *elapsed-time-since-last* checks against timestamps on `posts` and `profiles`, and login backoff has its own `login_attempts` table. A day tally cannot express "not again for 12 hours."
- **url_allowlist** — operator-curated domains whose links render as **clickable** (SPEC §7.2.3, §7.2.4), edited in admin. **Not a bare domain list.** Each row carries the host, the **admitting category** (convening / hosts-what-we-cannot-host-or-cannot-host-at-a-useful-size / messenger handoff — SPEC §7.2.3, so the operator's own filter stays legible over time), a **surface scope** (new in 1.29: posts-and-comments / cards / both — messenger domains are card-only, so a WhatsApp link is a hyperlink on a contact card and a copy box in a post), and **redirector-rejection patterns**: the paths and query parameters on that host which must be refused even though the host is allowed. Rows are seeded with the known cases (`youtube.com/redirect`, `google.com/url`, and any `?q=`/`?url=`/`?redirect_uri=`-style parameter carrying an absolute URL). A row may also be marked inactive rather than deleted, so an operator can withdraw a domain without losing the reason it was added.
- **url_blocklist** (new in 1.29) — operator-curated domains that may not be delivered **in any form**, the one link outcome that is an actual refusal (SPEC §7.2.3, §7.2.4). Each row carries the host and an **active** flag, and rows are retired by clearing `active`, never deleted — the same "deactivating beats deleting" discipline as `url_allowlist` above and `reaction_phrases` below, and for the same reason: the row is the record of why the domain was blocked. It is a **separate table rather than a flag on `url_allowlist`**, because the two lists answer different questions (*what becomes a link* versus *what is refused*) and nothing is ever on both; a single table with a tri-state column invites a row that is somehow both. **Read by the same shared classifier as the allowlist** (§7), so the blocklist check runs everywhere the allowlist check runs — including the edit path, which is where SPEC §7.8 invariant 4 says this class of control gets defeated in practice.
...
**The operator-curated *sets* of SPEC §14 are the exception, and they are tables (v1.25).** `HASHTAG_VOCAB`, `NAME_BLOCKLIST`, the URL allowlist, the **URL blocklist** (new in 1.29) and `REACTION_SET` (new in 1.25) are edited by the operator in Django admin while the service is running (BUILD_PLAN Step 13.3), so each is the **contents of a table above**, not a Python literal: the SPEC §14 name refers to the table's rows. This was true of the first three from the first draft and was never stated; stating it is what stops a builder putting a curated list in `constants.py` and then discovering the admin editor has nothing to edit. The distinction is the one SPEC §14's ✎ mark already draws: a **number** the founder tunes at deploy time is a constant, a **list the operator maintains** is data.

**Two more belong on the data side and have no row above yet — recorded rather than invented (v1.25).** `THEME_SET` and `DEFAULT_AVATAR_SET` are both operator-curated and both get an admin editor (BUILD_PLAN Step 13.3), but §3.5 describes a theme as a named set of CSS variables and §4 gives neither a table. Whether a theme is a database row or a file the operator deploys is a real question with a real answer, and it is not this session's: it is flagged here and queued for the sync so that a builder reaching Step 13.3 finds the question already asked rather than resolving it by guessing.

**Single-source rendering helpers.** The visibility engine (§5) is the most important instance of a general rule, not the only one. **Five** things are rendered by **exactly one function each**, called by every surface and reimplemented by none:
...
| **Link renderer (new in 1.29, SPEC §7.2.4)** | A URL in stored text, as a hyperlink or as a copy box, from the §7 classifier's answer | Three outcomes across five surfaces (feed, blog, comments, extended bio, contact card) is fifteen chances to render one of them slightly differently. The failure that matters is a surface that linkifies what the classifier called `PLAIN`, which is silent and looks right |

The time helper carries one prohibition the others do not: **it emits the phrase and nothing else.** No `title` attribute, no `<time datetime="…">`, no data attribute carrying the true timestamp (SPEC §7.5.1). The exact value is deliberately absent from the interface *and* the markup — a tooltip would fail SPEC §16.4's hover ban, and a `datetime` attribute would ship in page source the precision the rule exists to withhold. Its two documented exceptions — the absolute-days expiry countdown (SPEC §7.5) and account/security events (SPEC §4.6.1) — are separate call sites that format absolute time on purpose, not flags on the helper.
...
The module implements, from SPEC: §5.4 blocks (checked first, always, both directions), §7.4 audience snapshot + current friendship, §8.1 comment visibility (the post's audience minus blocks — new in 1.26), §9.2 profile tiers, §10.3 the contact cascade with deny-beats-allow, §11.3 the three-condition hashtag gate evaluated live, §11.5 friend-list filtering. "Preview-as" (SPEC §9.5) is the same code with a substituted viewer — plus one guard: preview mode must be strictly read-only.
...
**The contact-card report freeze is a caller of this module, and getting it wrong is a disclosure rather than a bug (new in 1.30; SPEC §13.2, §10.4).** Reporting a card item freezes *the card as that reporter received it*, and it does that by calling **`visible_contact_card(reporter, owner)` once, at submit, with the reporter as the viewer** — then storing the answer as a value (§4). Two shortcuts are available and both are wrong in the same direction:

- **Reading `contact_items` and `contact_overrides` and resolving the cascade in the report code.** This is Decision 4 broken in a new place, and the output is not merely unauthoritative — it is **every item the owner has**, including the ones this reporter was never shown, handed to the operator as evidence. A second implementation of the cascade turns a report into a disclosure channel.
- **Calling the engine with the wrong viewer** — the operator, or the owner, or whoever happens to be in `request.user`. §5.3 already requires the viewer to be an explicit argument for exactly this class of reason; here the wrong viewer produces a plausible-looking card belonging to nobody. The reporter is the viewer because the reporter is who received it.

The freeze runs in the ordinary submit request, so §5.3's memo behaves as it does anywhere else, and filing a report writes only to `reports` — none of the relations in §5.3's write rule — so nothing is invalidated. **The card page itself (SPEC §10.4) is the same call on the ordinary read path**, which is the point: one function answers both, and the frozen copy is that function's output at a moment rather than a second opinion about it.

**Comments and reactions are narrower than their post, and they are lists — which is how the rule reaches them (v1.26).** SPEC §8.1's comment audience is the post's audience **minus anyone blocked with the comment's author**, and SPEC §8.2's reaction line is visible to one person **unless a block stands between that person and the reactor**. Both are strictly smaller questions than `can_see_post`, and neither may be answered by the surface that happens to be drawing it. Neither gets bespoke machinery either: the **comment list under a post** and the **reaction line on a single-post view** are lists, so they take their base queryset from this module like every other list on the platform (§5.4), and `can_see_comment` is that same rule over a set of one — for the single comment reached from a notification link or written into an export. **A reaction needs no function of its own**: its audience is one person, so a block does not narrow it but empties it, and the engine returns the line or returns nothing. Before 1.26 SPEC stated that a comment was visible to *exactly* the people who could see its post, so this whole class of question looked already answered; the correction is SPEC's, and this is its implementation.
...
| **The answers themselves** — `can_see_post`, `can_see_comment`, `can_see_profile_tier`, `visible_contact_card`, `are_connected`, `can_act` | the **full argument tuple, viewer first** | free after the above | The inputs cannot change mid-render (but see the write rule below) |
...
**The singular functions do not go away; they become the item-level face of the same code.** Decision 4's singular questions remain the API for a single post, a single comment, a single profile, a single card — the single-post view, notification delivery (§4), the permission-checked image view (Decision 3), the data export (SPEC §4.9). Where a singular answer can be expressed as a plural one over a set of one, **it is implemented that way**, so that each rule has one implementation and not two.
...
|---|---|---|
| `expire_content` | hourly | Delete posts/comments > 90 days (unpinned), their images and their reactions; **plus every reaction more than 90 days past its own `set_at`, including reactions on pinned posts** — a reaction has a clock of its own and a pinned post never expires to cascade from (SPEC §7.5, §8.2.1) |
| `purge_moderation` | daily | Purge frozen copies past their date; hard cap 90 days (SPEC §13.3) |
| `replenish_invites` | daily | +1 invite to anyone `INVITE_REPLENISH_DAYS` = 30 days past last replenish, cap 5 (SPEC §4.2); expire 14-day-old invites back to budget (SPEC §4.1) |
| `process_deletions` | daily | Erase accounts past 30-day grace; write anonymized invite-tree stubs (SPEC §4.7, §4.3) |
| `inactivity_sweep` | daily | Send `INACTIVITY_WARN_DAYS` warnings (180/365/670/700 days since last login); delete at `INACTIVITY_DELETE_DAYS` = 730 (SPEC §4.8). **Days, not calendar months** |
| `reset_rate_counters` | daily | Clear yesterday's rate-limit counts (SPEC §13.6). Leaves the timestamp-based controls alone — feed-post spacing and the bio/photo cooldowns are elapsed-time checks, not counters (§4) |
| `expire_notifications` | daily | Delete notifications whose subject no longer exists (expired or deleted post, deleted comment, deleted account), **any left with no actors at all** — every comment deleted, every reaction removed or expired (SPEC §12.2) — and any older than `CONTENT_TTL_DAYS`. Without this, the feed accumulates links to content the 90-day promise has already destroyed (SPEC §7.5, §12) |
...
- **Authentication and anti-phishing (SPEC §4.6.1):**
  ...
  - *Breach-password check.* At registration and password change, the backend makes a **server-to-server** k-anonymity range request to the Have I Been Pwned Pwned-Passwords API (send the first 5 hex characters of the password's SHA-1; match the returned suffix list locally). Only a hash prefix ever leaves the server — never the password, never any user identifier — and it is a backend HTTPS call, not a browser script, so it is SPEC §15.2-clean. Fail-open on API outage (a third party being down must never block a signup), logged.
...
- **Link classification (SPEC §7.2.3, §7.2.4) — a classifier, not a gate (rewritten 1.29):** SPEC's link policy has **three outcomes**, so the shared function returns one of three answers rather than pass/fail: **`CLICKABLE`** (allowlisted, in scope for this surface), **`PLAIN`** (on neither list — rendered as an inert copy box), or **`REFUSED`** (blocklisted — the save fails). It is still one function, still a **parser and not a substring check**, and still called by every surface that accepts text: feed posts, profile posts, comments, the extended bio, and contact-card items — **on every save, create and edit alike** (SPEC §7.8 invariant 4). A classifier wired only into the create path is a **defective implementation**, and a quiet one: it would let an author publish clean text, pass the check, and then edit a **blocklisted** link into the stored post, defeating the one outcome the platform actually forbids without ever raising an error. The same "revalidate on every save" rule governs length caps, whitespace normalization, hashtag-vocabulary membership, contact-card label screening, and image EXIF-stripping on a replaced image. Practically, this means content validation belongs to the model/form layer that both paths share, never to the create view. The classifier must:
  1. Parse the URL properly (never regex-match a raw string).
  2. **Check the blocklist first** (§4). A blocklisted host is `REFUSED` before anything else is considered, so no ordering of the remaining rules can produce a clickable blocklisted link.
  3. Match the **host exactly or as a subdomain of an allowlisted host** — never a substring, never a suffix test (`evil-youtube.com` and `youtube.com.attacker.net` must both fail). A miss is `PLAIN`, not an error.
  4. Require `https` **for `CLICKABLE`**. An `http://` URL on an allowlisted host is `PLAIN`, not refused: it is delivered as inert text like any other unapproved address, and nothing on this platform ever links to plaintext HTTP.
  5. **Reject the row's redirector patterns** (§4) — outcome `PLAIN`. This step is not optional: `youtube.com/redirect?q=…` and `google.com/url?q=…` pass any host check yet hand the reader to an arbitrary attacker page, which would silently defeat the whole control. **A host-only implementation is a defective implementation of this section.** As a belt-and-braces rule, any otherwise-allowed URL whose query string contains an absolute `http(s)://` URL is downgraded to `PLAIN` regardless of host.
  6. Refuse to treat URL shorteners as `CLICKABLE` categorically — they make the destination unknowable, so no shortener is ever allowlistable (SPEC §7.2.3).
  7. **Take the surface as an argument** (`post` | `card`) and honour the row's scope (§4): an allowlisted row scoped to cards returns `CLICKABLE` on a contact card and `PLAIN` in a post. One function, one policy, two answers — never two functions, which is how the two surfaces drift apart.
  ...
  **Note what moved and what did not.** Before 1.29 a non-allowlisted URL was rejected at composition; it is now delivered as text. The parsing, host-matching and redirector rules are **unchanged and still load-bearing** — they now decide *clickability* rather than *delivery*, and SPEC §4.6.1's anti-phishing claim was narrowed to match rather than left overstated. The `PLAIN` outcome is **not an error condition anywhere in the stack**: it produces no form error, no message and no error styling (SPEC §7.2.4), and a builder who implements it as a warning has built the wrong thing.

- **The copy box (SPEC §7.2.4) is server-rendered, and the copy control is the only script on it.** The address is rendered as text inside its own container — wrapped break-anywhere so a long URL cannot break the 320 px reflow guarantee (SPEC §16.3), which is a real risk because URLs offer no ordinary break opportunities. The clipboard write is a few lines of progressive enhancement on a real `<button>` with a distinct accessible name and a polite live-region confirmation; **where script is unavailable the address is still fully present and selectable**, which is the whole reason the value is text on the page rather than something a script fetches.
- **No third-party anything in pages:** no CDN assets, no fonts from Google, no analytics, no embeds (SPEC §7.2, §15.2). Allowlisted links are rendered as **clickable links only, never embedded or preview-carded** — an embed would load the external service's scripts into our page. A build-plan checklist item: load any page and confirm the browser makes **zero** requests to foreign domains.
- **Rate limits** (SPEC §13.6) enforced in the database per user/action/day. **Login attempts** get a stricter, security-specific control (SPEC §4.6.1): per-account *and* per-source-address failure counters in the database with exponential backoff and temporary lockout (`LOGIN_ATTEMPT_LIMIT`, `LOGIN_LOCKOUT_MINUTES`), complemented by OS-level fail2ban on the SSH surface. No CAPTCHA (rejected — SPEC §4.6.1, §14 below).
...
- **Unguessable IDs** in all URLs (SPEC §9.3); every request permission-checked (Decision 3).
...
- **Logs:** operational/error logs only, IP-lean, short retention — aligned with SPEC §15.2's "minimal and short-lived."
...
- **It is not a page** — no HTML, no template, no theme — which is why it does not contradict
  ...
  written down here rather than left for a reader to reconstruct. **Settled 2026-08-18:** SPEC §2
  does not name the route, because §2's rule is about *pages* and this is not one; SPEC §2 now
  points here rather than restating the argument, so the endpoint has exactly one description and
  it is this paragraph. What a future unauthenticated route has to meet is the test above — it
  answers nothing else, touches no user table, and is not a page — not this route's precedent.
...
- **What stands in for it here, all of it already built:** everything but three routes and
  ...
  (§7.1); fail2ban guards SSH; SPEC §13.6's rate limits cap what any account can do in a day; and
...
- **Dense automated tests on the visibility engine** — blocks, tier boundaries, cascade conflicts, hashtag-gate edge cases, snapshot-plus-current-friendship, and **the comment a block hides on a post both parties can see** (SPEC §8.1, §5.4, v1.26 — the case with no analogue in `can_see_post`, which every post-level test passes while the comment leaks). This is where a bug = a privacy breach.
...
- **Automated tests on the lifecycle jobs** — expiry, deletion-with-stub, invite replenishment, notification expiry. A bug here breaks a promise made to users (SPEC §7.5's "nothing outlives 3 months"). **One case is named because it is the one that fails silently: a reaction backdated 91 days on a *pinned* post must be gone after `expire_content` runs** (SPEC §8.2.1). A job written from the parent-cascade alone passes every other row in the suite and fails only this one, and only in production, where the pinned post is years old.
...
- **A named test for the two helpers that had none (v1.18)** — the blast-radius rule applied to the time helper's siblings, and both are small:
  ...
  - **The link renderer** (SPEC §7.2.4, new in 1.29) — the same shape of truth table, over the classifier's three answers **and the surface**: an allowlisted post-scoped host is a hyperlink in a post; a **card-scoped** host (a messenger domain) is a hyperlink on a card and a **copy box in a post**; anything unlisted is a copy box on both; a blocklisted host never renders at all because the save was refused. Two assertions matter more than the rest and are easy to omit: **a copy box carries no error markup of any kind** (the whole difference between the two non-clickable outcomes is that one is a refusal and the other is not), and **no surface linkifies what the classifier called `PLAIN`** — the failure that looks right in a browser.
...
- **Tests on the edit path specifically** — that editing re-runs the link classifier (post a clean body, edit in a **blocklisted** link, assert refusal; edit in a merely non-allowlisted one, assert it is accepted and rendered as a copy box with no error), that `edited_at` never affects feed order or expiry, that an edit writes no `post_audience` rows, and that a post author cannot reach an update path for someone else's comment. These are small tests guarding SPEC §7.8's five invariants, and invariant 4 is a security test, not a feature test.
- **Tests on notification coalescing and follow delivery** — that a second event updates the open unread row rather than inserting a duplicate, that removed reactions and deleted comments vanish from a rendered notification, **that a notification whose last actor drops out is gone rather than blank** (SPEC §12.2), that no query returns a follower list or an unread count, and that unfriending/blocking/a dropped hashtag silently ends delivery to a follower who still has a `post_follows` row. The last one is a visibility-engine test wearing a notification costume.
- **Ordinary features** (rendering, forms): light tests plus human use. The prototype's friendly first users are the beta test.
- **Accessibility (SPEC §16)** gets two automated tests plus a human pass, because two thirds of it *is* machine-checkable and the rest genuinely isn't. **The two automated ones are continuous and the human one is periodic (v1.22)** — that distinction is the whole of SPEC §16.5's three clocks, and it is spelled out here because a reader of this section could previously have concluded, wrongly, that all of it was a Phase 16 event:
  - a **contrast test** over every `THEME_SET` combination (§3.8) — pure Python, no dependency, no browser, fails by theme name. Built at build-plan Step 8.2 alongside the themes themselves, so it has been continuous since the day a theme first existed. A theme added in year two that fails it fails the build the day it is added.
  - **template smoke tests** asserting the mechanical invariants on rendered HTML: every `<img>` has an `alt` attribute (possibly empty with `is_decorative`), every input has an associated label, every page has one `<h1>` and a `<title>`, no `tabindex` above 0, no `<div>` carrying a click handler where a `<button>` belongs. **These are built at build-plan Step 2.5, with the base template and the shared partials, and their page list grows with every phase that adds a page (v1.22).** Until 1.22 this document specified them while no build step created them, and the build plan mentioned them only inside the pre-launch audit — so a check described here as a *test* was in practice a *gate*, run once. That is now fixed at the only place it can be: the step that builds the first pages worth asserting anything about. A smoke-test page list that stopped growing in Phase 2 passes forever and checks nothing, which is why the instruction to extend it is repeated in the build plan's cross-phase rules.
  - The **human pass** — keyboard-only, screen reader, 200% zoom, 320px reflow — is a build-plan gate (Step 16.5) rather than a unit test, and **it repeats on SPEC §16.5's re-audit triggers rather than only before launch**. No automated tool catches "the focus order is bewildering"; roughly a third of WCAG failures are only findable this way. **It is run by a qualified tester, not by the founder** (SPEC §16.5.1; the tester is booked at build-plan Step 15.3). What stays the founder's is the automated set above, the milestone scan below, and one keyboard-and-reflow spot check a year.
- **The axe/`pa11y` scan: local tooling, on a milestone cadence — decided 2026-08-18, and recorded here so it is not re-proposed.** An **axe** browser extension, or `pa11y`/`axe-core` run against a locally served instance, is genuinely useful, and 1.22 asked whether it could become continuous like the two tests above. The answer has two halves. **It joins the cadence:** it runs at each build-plan phase milestone (BUILD_PLAN §0.5), not once before launch, because a tool that catches a third of real problems is worth running a dozen times. **It does not join the test suite, and it is never an application dependency:** it needs a running server and a headless browser, which means a Chromium download and a Node toolchain sitting in a Python project's test path — a dependency in every sense that matters, even though nothing it touches ever reaches a user's browser. Note which constraint is doing the work: SPEC §15.2 is satisfied *either* way, since nothing accessibility-related ships to the browser in any of these arrangements; what rules it out of the suite is dependency weight and flakiness alone, and that is sufficient on its own. Any future proposal to make it a *project* dependency still needs founder approval under the build-plan dependency rule, and this paragraph is the standing answer to it.
...
6. **Availability posture and the three calls of 2026-08-17** (§7.2–§7.4, §11.1): the session that wrote them put three questions to the founder, and one answer went against the recommendation, which is why it is recorded here in full rather than summarized.
   ...
   Recorded beside them as fact rather than as open questions (item 6): **no new dependency and no new outside party** were added (§3's stack is untouched; the declined monitoring service was the only candidate, and the privacy policy therefore gains no processor), and **SPEC was not edited**. One SPEC-boundary question was left open on purpose rather than answered here: whether SPEC §2's "no public pages except login/registration/invite-acceptance" should name `/healthz`. §7.3 argued it need not, an endpoint serving no page not being a page, and the call belonged to prompt 08. **Answered 1.24 — founder decision 2026-08-18: it need not.** SPEC §2's rule is about pages; `/healthz` is not one; SPEC §2 gained a pointer to §7.3 rather than a restatement of it, so the endpoint keeps one description in one place. Left open beside it, and *not* decided: SPEC §2's list is in fact narrower than the built system in a second way that has nothing to do with this route — the privacy policy and accessibility statement are linked from login (build-plan §15.1, §15.2) and SPEC §16.1's own scope line names a different three pages than §2 does. Correcting a normative list is a content change, so it is queued rather than made here.

7. **Accessibility after launch, and the operator console — four founder decisions of 2026-08-18** (§3.8, §9; SPEC §16.1.1, §16.5, §16.5.1). The occasion was a reviewer's claim that accessibility testing here is manual and unscanned, which was **wrong** — Step 16.5 pass 1 already runs a scan, and this section already specified a contrast test and template smoke tests. What survived the check was the second half of the objection: **every one of those checks was a gate, run once, immediately before launch**, in a project whose build plan ends at Phase 17 while features keep arriving. The four calls:
   - **Conformance runs on three clocks, not one — APPROVED 2026-08-18.** Continuous (the two automated tests, every change forever), per-milestone (the scan), periodic (the human passes, on triggers). Checking the claim exposed a real defect on the way: **the template smoke tests had no build step at all** — specified here since the document was written, mentioned in the build plan only inside the pre-launch audit. They now belong to Step 2.5. This is worth recording as the pattern it is: a test this document specifies and the build plan never builds is indistinguishable, from inside either document, from a test that exists.
   - **The axe/`pa11y` scan moves to a milestone cadence but stays out of the test suite — APPROVED 2026-08-18**, reasoning in §9. The constraint that decided it was **not** SPEC §15.2: nothing accessibility-related ships to a browser under any of the options considered. It was dependency weight — a headless Chromium and a Node toolchain in a Python test path.
   - **The human passes are run by a qualified tester, not the founder — FOUNDER DECISION, brought to this session already taken.** A screen-reader pass is a skill, not a setting, and it had been the only quality gate before launch. Preference recorded in SPEC §16.5.1: a daily screen-reader user, paid for their time, before a professional auditor. Its real cost is a **scheduling dependency** — build-plan Step 15.3 now books the person, because Phase 16 had quietly assumed the founder was available to himself.
   - **The operator console is a bounded commitment — APPROVED 2026-08-18**, over an explicit exemption and over full scope. Full scope was weighed and rejected as a **stack-level reversal**: it means a hand-built operator interface, and §3.1 leg 2 makes the admin-for-free one of the five reasons Django was chosen. The bounded subset (keyboard, focus, contrast, labels, text errors, over the four weekly surfaces) is roughly what the admin already delivers and is verified in a sitting already being paid for. SPEC §16.1.1 carries the decision and the future-operator remedy: first-party operator pages if the role ever falls to someone the admin excludes, never an overlay.

   Recorded beside them as fact rather than as open questions: **no new dependency, no new outside party and no new infrastructure** (the scan is a tool on the founder's Mac; the tester is a person, not a processor — the launch audit runs before Step 17.1's reset to zero, so nothing they see belongs to anyone, and a **post-launch re-audit puts them on an ordinary member account whose only friend is the founder**, which makes them a member seeing a member's view rather than anything the privacy policy must name. There is one server and no staging copy, so this is decided rather than improvised — SPEC §16.5.1 carries it). **SPEC was edited, and correctly so:** the scope of a conformance claim and who is competent to verify it are behaviour and commitment, not implementation.

8. **Reactions get a clock, and `REACTION_SET` gets a table (§4, §6; SPEC §8.2.1–§8.2.3).** From project version 1.25. The finding was a reviewer's: reactions on a pinned post could never expire, because the only thing that deleted a reaction was the deletion of its parent and a pinned post is never deleted. The behavioural answer is SPEC's and was made there — **a reaction expires 90 days after it was last set, on a clock of its own** — and this document carries the two consequences that are implementation:
   - **`reactions` gains `set_at`, and `expire_content` sweeps on it** rather than only cascading from a deleted post. Recorded as a shape lesson rather than a bug fix: every other expiring thing in this system hangs off a parent that expires, and the reaction was the one child whose parent could outlive it. A job written from the cascade alone is correct for every row it will ever see in testing and wrong for the only case that matters.
   - **`REACTION_SET` is a table (`reaction_phrases`) with an `active` flag, not a constant**, because SPEC §8.2.3 requires that a retired phrase stop being offered while existing reactions keep rendering it unchanged — which a Python literal cannot express and an admin editor cannot edit. The migration question that would ordinarily follow ("what happens to reactions using a phrase that no longer exists?") **has no answer to build**: reactions expire within 90 days, so the set drains itself. That is worth naming as the general pattern it is — on this platform, expiry is frequently the cheapest migration available.

   Recorded beside them as fact rather than as open questions: **no new dependency, no new infrastructure, and no new job** — `expire_content` and `expire_notifications` already existed and each gained a clause. **SPEC was edited, and correctly so:** a lifetime, what a reaction looks like on screen, and who may see one are behaviour.

9. **Comment visibility is a question of its own (§5, §9; SPEC §8.1, §5.4).** From project version 1.26, handed over by the design session that resolved three SPEC contradictions. SPEC had asserted that a comment is visible to *exactly* the people who can see its post, which SPEC §5.4 contradicts — a block removes the blocked party from the comment's audience while leaving them in the post's. The behavioural correction is SPEC's and was made there; this document carries the two consequences that are implementation:
   - **`can_see_comment(viewer, comment)` joins Decision 4's list**, and the comment list under a post joins §5.4's set of engine-owned querysets. The defect was not a wrong answer anywhere — it was that **no answer had an owner**: with the two audiences asserted identical, no check was ever specified, so each template rendering a comment either applied the block rule itself or silently did not. That is Decision 4 being broken by a *gap* rather than by an inlined query, which is the harder form to see in review, and worth recording as its own shape lesson: **a rule the specification says is redundant produces no code and no test, and looks identical to a rule that is handled.**
   - **Reactions need no function.** SPEC §5.4 covers "comments **or** reactions", and the case is real — a block after the fact must remove a reaction already given from its recipient's view — but a reaction's audience is exactly one person, so the block empties it rather than narrowing it. The reaction line takes its contents from the engine as a list and renders nothing when the block stands. Recorded so that nobody adds `can_see_reaction` later on the assumption it was overlooked.

   Recorded beside them as fact rather than as open questions: **no new dependency, no new infrastructure and no new table** — this is one function and one queryset inside a module Phase 4 already builds. **BUILD_PLAN was deliberately not touched**, on the 1.19 and 1.25 precedent: Step 4.1 names "the five functions of ARCHITECTURE Decision 4" and there are now six, and Step 4.2's suite needs the block-hides-a-comment case; both are itemized in `prompts/09-sync-arch-and-buildplan.md` §L for the sync session.

10. **The link validator becomes a classifier, and a blocklist joins the allowlist (§4, §7; SPEC §7.2.3, §7.2.4).** From project version 1.29. The founder replaced a two-outcome rule (allowlisted or rejected) with a three-outcome one (clickable, plain copy box, refused). The behavioural decision is SPEC's and was made there; this document carries four consequences that are implementation:
   - **One function, three answers, and the surface as an argument.** The shared validator becomes a classifier returning `CLICKABLE` / `PLAIN` / `REFUSED`, and takes the calling surface (`post` | `card`) because allowlist rows are now scoped — messenger domains are clickable on a contact card and plain in a post. **Two functions, one per surface, was the obvious alternative and is the defect**: the parsing, host-matching and redirector rules are the security-critical part, and a second copy of them is a second place for the redirector check to be missing. The parse-and-match rules themselves did not change at all; what changed is the name of what they decide.
   - **The blocklist is a separate table, and it is the *second* control to depend on SPEC §7.8 invariant 4.** Before 1.29 a create-only validator meant a disallowed link could be edited in after publication — bad, but the link was never clickable anyway. Now it means a **blocklisted** domain can be edited in after publication, defeating the one outcome the platform actually forbids. The invariant did not change; its stakes did, and §7 says so where a builder will read it.
   - **`PLAIN` is not an error, and that is an implementation rule as much as a copy rule.** It produces no form error, no message, no error styling and no logging. The likely defect is a builder treating "not on the allowlist" as the failure case it used to be — which produces a technically working platform that scolds its users on every ordinary post (SPEC §7.2.4).
   - **`contact_items` gains a `label` column and its `messenger-link` kind becomes `link`.** Clickability is read from the allowlist at render time rather than stored on the item, so an operator scoping a row changes every existing card at once instead of leaving old rows frozen at the policy that applied when they were saved. The label is screened at every save like `short_bio`, and **link items are gated on the card-report design that does not exist yet** (SPEC §10.2) — recorded here so the gate is visible from the data model, not only from the build step.

   Recorded beside them as fact rather than as open questions: **one new table (`url_blocklist`), one new column on `contact_items` plus one renamed kind value, no new dependency, no new infrastructure and no new job.** The copy box needs no library — it is server-rendered text plus a few lines of progressive enhancement on a real button (§7). **SPEC was edited, and correctly so:** what a reader sees when a friend posts a link, and what is refused outright, are behaviour.


11. **A contact card becomes a reportable object, and the freeze is the engine's output rather than a second cascade (§4, §5; SPEC §10.4, §13.2, §13.2.1).** From project version 1.30. SPEC v1.29 put an author-chosen address and an author-written label on a surface with no report action; this is the path it needed, and the behavioural decisions are SPEC's. Four consequences are implementation, and three of them are corrections to this document rather than additions:
    - **`card_requests` stored the wrong thing, and the wording is what would have built it.** "Who asked whom, when, **what was auto-answered**" reads as *store the resolved card*, which is the send-time freeze SPEC §10.4 rejected: a copy of every card every friend ever asked for, held for the life of the friendship, against a report that will almost never come — and a stored answer cannot lose an item the owner switched off, which is the whole discipline of SPEC §10.3. The row now records only that the request happened. **The card page re-resolves on every visit**, which costs one memoized engine call (§5.3) and is what makes the report action reachable at all.
    - **The frozen copy is a value, not a reference, and it holds no resolution trace.** An ordered list of `{kind, value, label}` plus the position of the reported item. A foreign key back to `contact_items` would be defeated by the owner deleting the row — the first move a reported person makes — and capturing the cascade that produced the version would disclose the owner's private group structure (SPEC §6) to answer a question the operator did not ask.
    - **The freeze calls `visible_contact_card(reporter, owner)`; it does not re-resolve.** This is the sharpest item here because the failure is not a wrong answer. A report path that reads `contact_items` directly captures **every item the owner has**, including the ones that reporter was never shown, and files them as evidence. Decision 4 is usually defended on drift; here it is defended on disclosure, and §5.1 says so where a builder will read it.
    - **The upheld action deletes the `contact_items` row, whole.** Never a partial edit of one — blanking a label is the operator writing into somebody's account, and this platform's operator tools are remove, warn and ban. Nothing on `contact_overrides` is touched: that is configuration, not content.

    Recorded beside them as fact rather than as open questions: **no new table, no new column beyond the report's own frozen copy, no new job, no new dependency and no new infrastructure.** `purge_moderation` (§6) already purges frozen copies on the 30/90-day rule and needs no file branch, because a frozen card is text and nothing on a card is an image. **SPEC was edited, and correctly so:** whether a received card is a page or a one-time reply, what an operator may see of it, and what an upheld report removes are all behaviour.
