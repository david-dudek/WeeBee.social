...
| `post-editor.html` | M6 | §7.2.3, §7.8 | Line 31 says an edit is re-checked against *"the URL allowlist"*. Incomplete since 1.29: the **blocklist** is checked by the same shared validator on **every save path, create and edit alike** (§7.8 invariant 4), and §7.2.3 says in terms that *"a create-only implementation is a defective implementation of this section."* The line must name both lists. Copy only — no new control. |
