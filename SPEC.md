...
**Project version:** 1.29 · 2026-09-07 · DRAFT — not yet founder-approved
**This file last changed in:** 1.29 (new §1.5 names **the Delegation Principle**; §7.2.3 rewritten as two link lists with surface scope; new §7.2.4 gives the three link outcomes and the copy box; §10.2 gains card links and labels; §4.6.1, §13.1, §9.4, §7.5, §7.8, §13.5, §14, §16.3, §17 reconciled)
...
**A card is not reportable, and link items must not ship before it is (v1.29).** Report actions exist on posts, comments and profiles (§13.2); a contact card is not a reportable object anywhere in this document. That was tolerable while a card held only a phone number, an email address and a messenger link from an allowlisted domain. It is not tolerable once a card carries an author-chosen address **and an author-written label**, delivered to a friend who requested the card — the same shape of gap §13.2 closed in v1.16 when it put a report action on the friend-request card, and for the same reason: *a report the recipient cannot reach is not a defence.*

The design is genuinely harder than the friend-request case, which is why it is its own piece of work rather than a clause here. **A card is per-requester** (§10.3): what the operator must see is the version the reporter actually received, not the version the owner is showing now, which they can change the moment a report is filed. That points at a §5.2-style **snapshot** — the card as delivered, frozen at report time — and at a new target type in the report queue (§13.2, §13.3). **The founder's decision (2026-09-07): the link and label rules above are specified now; the report path is designed in its own session (`prompts/15-reporting-a-contact-card.md`), and BUILD_PLAN Step 9.1 does not ship card link items until it lands.** The dependency is a build gate, recorded in the step, not a version number.
...
- **Repeated controls carry distinct accessible names (v1.16).** A list of posts produces many identical "read more" controls, and a screen reader's element list of twenty entries reading "Read more" is a dead end. Each is a real `<button>` (it navigates nowhere) with `aria-expanded`, a unique name — *"Read more of David's post from a few days ago"* — via visually hidden text, and focus that stays on the control as it becomes "Show less". The same rule governs the gallery's per-image controls, which take their names from the image's own alternative text (§9.4); the **reaction picker** of §8.2.2, which produces one control for the post plus one for every comment on a single-post view; and the **copy control on a link's copy box** (§7.2.4, v1.29), of which a single feed page may carry many — its accessible name has to say which address it copies, and the copy is confirmed in a polite live region.
